...
const C={bg:"#efe9e1",surface:"#ffffff",surfaceAlt:"#f6f3ee",field:"#ebe5dd",line:"#e4ded4",lineStrong:"#d8d0c4",text:"#2c2c30",textMute:"#8a857c",textFaint:"#a8a298",onDark:"#f6f3ee",orange:C.orange,orangeBg:"#fae7d8",green:"#1aa05f",greenBg:"#dff0e4",gold:"#c4901a",goldBg:"#f6ecd2",blue:"#2f7fd6",blueBg:"#dfeaf8",red:"#d64545",redBg:"#f8e0e0"};
...
body{background:${C.bg};color:${C.text};font-family:'Barlow',sans-serif;font-size:15px;line-height:1.4;overscroll-behavior:none;}
...
const base={display:"flex",flexDirection:"column",height:"100dvh",maxWidth:480,margin:"0 auto",background:C.bg,overflow:"hidden"};
...
function MainApp({session,onLogout}){
  ...
  return(<><style>{css}</style>
    <div style={base}>
      <div style={{padding:"12px 16px 10px",borderBottom:"1px solid #e4ded4",background:C.surfaceAlt,flexShrink:0}}>
...
<button style={{...btn("ghost"),padding:"6px 9px"}} onClick={onLogout}>{Ic.logout}</button>
      ...
      <nav style={{borderTop:"1px solid #e4ded4",background:C.surfaceAlt,flexShrink:0}}>
        <div style={{display:"flex",borderBottom:"1px solid #e4ded4"}}>
          {TABS.slice(0,4).map(([id,label,icon])=>(
            <button key={id} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"7px 2px 7px",border:"none",background:"none",color:tab===id?C.orange:C.textMute,cursor:"pointer",fontSize:9,fontFamily:"'Barlow',sans-serif",fontWeight:600,letterSpacing:0.3,textTransform:"uppercase"}} onClick={()=>setTab(id)}>
              {icon}{label}
            </button>
          ))}
        </div>
        <div style={{display:"flex",borderTop:"1px solid #e4ded4"}}>
          {TABS.slice(4).map(([id,label,icon])=>(
            <button key={id} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"7px 2px 10px",border:"none",background:"none",color:tab===id?C.orange:C.textMute,cursor:"pointer",fontSize:9,fontFamily:"'Barlow',sans-serif",fontWeight:600,letterSpacing:0.3,textTransform:"uppercase"}} onClick={()=>setTab(id)}>
              {icon}{label}
            </button>
          ))}
        </div>
...
function WeightTab({data,onSave}){
  ...
  return(
    <div style={{padding:"14px 14px 32px"}}>
      <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:22,color:C.text,letterSpacing:0.5,marginBottom:16}}>WEIGHT & MEASUREMENTS</div>
        ...
        <>
          ...
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            <div style={statBox(C.orange)}>
              <div style={{fontSize:10,color:C.orange,fontWeight:700,letterSpacing:1,marginBottom:4}}>WEIGHT</div>
              <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:26,color:C.orange,lineHeight:1}}>{currentWeight}<span style={{fontSize:12,fontWeight:500}}>kg</span></div>
              {totalChange!==null&&<div style={{fontSize:10,color:totalChange<0?C.green:totalChange>0?C.red:C.textMute,marginTop:3}}>{totalChange>0?"+":""}{totalChange}kg total</div>}
            </div>
            {hasWaist&&latestWaist&&(
              <div style={statBox(C.blue)}>
                <div style={{fontSize:10,color:C.blue,fontWeight:700,letterSpacing:1,marginBottom:4}}>WAIST</div>
                <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:26,color:C.blue,lineHeight:1}}>{latestWaist}<span style={{fontSize:12,fontWeight:500}}>cm</span></div>
                {waistChange!==null&&<div style={{fontSize:10,color:waistChange<0?C.green:waistChange>0?C.red:C.textMute,marginTop:3}}>{waistChange>0?"+":""}{waistChange}cm total</div>}
              </div>
            )}
            {totalChange!==null&&!hasWaist&&(
              <div style={statBox(totalChange<0?C.green:totalChange>0?C.red:C.blue)}>
                <div style={{fontSize:10,color:C.blue,fontWeight:700,letterSpacing:1,marginBottom:4}}>CHANGE</div>
                <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:26,color:totalChange<0?C.green:totalChange>0?C.red:C.blue,lineHeight:1}}>{totalChange>0?"+":""}{totalChange}<span style={{fontSize:12,fontWeight:500}}>kg</span></div>
                <div style={{fontSize:10,color:C.textMute,marginTop:2}}>from {startWeight}kg</div>
              </div>
            )}
          </div>
          ...
          <div style={{...card,padding:14,marginBottom:14}}>
            ...
            {data?.targetWeight&&toTarget!==null&&(
              <div style={{marginTop:10,padding:"9px 12px",background:C.orangeBg,borderRadius:7,border:"1px solid #fae7d8",display:"flex",alignItems:"center",gap:8}}>
                {Ic.target}<span style={{fontSize:13,color:toTarget<=0?C.green:C.text}}>{toTarget<=0?"Weight goal reached! 🎉":`${Math.abs(toTarget)}kg to ${data.targetWeight}kg`}</span>
              </div>
            )}
          ...
          {weeklyAvgs.length>1&&(
            <div style={{...card,padding:"14px 8px 14px 4px",marginBottom:14}}>
              <div style={{padding:"0 10px 10px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:13,color:C.textMute,letterSpacing:1}}>WEEKLY TREND</div>
                {hasWaist&&(
                  <div style={{display:"flex",gap:4}}>
                    {[["weight","Weight"],["waist","Waist"]].map(([k,l])=>(
                      <button key={k} onClick={()=>setActiveChart(k)} style={{padding:"4px 10px",borderRadius:5,border:"none",cursor:"pointer",fontSize:11,fontWeight:700,background:activeChart===k?C.orange:C.field,color:activeChart===k?"white":C.textMute}}>{l}</button>
                    ))}
                  </div>
                )}
              </div>
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={chartData} margin={{top:4,right:16,left:0,bottom:4}}>
                  ...
                  <XAxis dataKey="week" tick={{fill:C.textMute,fontSize:10}} axisLine={false} tickLine={false}/>
                  <YAxis tick={{fill:C.textMute,fontSize:10}} axisLine={false} tickLine={false} domain={["auto","auto"]} width={36}/>
                  {activeChart==="weight"&&data?.targetWeight&&<ReferenceLine y={data.targetWeight} stroke={C.green} strokeDasharray="4 4" strokeWidth={1.5}/>}

                  <Tooltip contentStyle={{background:C.field,border:"1px solid #d8d0c4",borderRadius:8,color:C.text,fontSize:12}} formatter={(v)=>[v?`${v}${activeChart==="weight"?"kg":"cm"}`:"No data","Avg"]} labelFormatter={l=>`Week: ${l}`}/>
                  {activeChart==="weight"&&<Line type="monotone" dataKey="avg" stroke={C.orange} strokeWidth={2.5} dot={{fill:C.orange,strokeWidth:0,r:3}} activeDot={{r:5}} connectNulls/>}
                  {activeChart==="waist"&&<Line type="monotone" dataKey="waist" stroke={C.blue} strokeWidth={2.5} dot={{fill:C.blue,strokeWidth:0,r:3}} activeDot={{r:5}} connectNulls/>}
                </LineChart>
              </ResponsiveContainer>
            </div>
          )}
            ...
            <div style={{marginBottom:14}}>
              ...
              <div style={card}>
                {[...weeklyAvgs].reverse().map((w,i)=>{
                  ...
                  return(
                    <div key={w.week} style={{padding:"10px 14px",borderTop:i===0?"none":"1px solid #efe9e1"}}>
                      <div style={{display:"flex",alignItems:"center"}}>
                        <div style={{flex:1}}>
                          <div style={{fontWeight:600,fontSize:13,color:C.text}}>Wk of {fmtDate(w.week)}</div>
                          <div style={{fontSize:11,color:C.textMute,marginTop:1}}>{w.entries} {w.entries===1?"entry":"entries"}</div>
                        </div>
                        <div style={{textAlign:"right"}}>
                          <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:18,color:C.orange}}>{w.avg}<span style={{fontSize:10,fontWeight:400}}>kg</span></div>
                          {diff!==null&&<div style={{fontSize:10,color:diff<0?C.green:diff>0?C.red:C.textMute}}>{diff>0?"+":""}{diff}kg</div>}
                        </div>
                        {w.waistAvg&&<div style={{textAlign:"right",marginLeft:16}}>
                          <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:18,color:C.blue}}>{w.waistAvg}<span style={{fontSize:10,fontWeight:400}}>cm</span></div>
                          {waistDiff!==null&&<div style={{fontSize:10,color:waistDiff<0?C.green:waistDiff>0?C.red:C.textMute}}>{waistDiff>0?"+":""}{waistDiff}cm</div>}
                        </div>}
                      </div>
                    </div>
                  );
          ...
          <div style={card}>
            {displayed.map((e,i)=>{
              ...
              return(
                <div key={e.date} style={{display:"flex",alignItems:"center",padding:"9px 14px",borderTop:i===0?"none":"1px solid #efe9e1"}}>
                  <div style={{flex:1}}>
                    <div style={{fontWeight:500,fontSize:14,color:C.text}}>{fmtDate(e.date)}</div>
                    {diff!==null&&<div style={{fontSize:11,marginTop:1,color:diff<0?C.green:diff>0?C.red:C.textMute}}>{diff>0?"+":""}{diff}kg</div>}
                  </div>
                  <div style={{textAlign:"right",marginRight:8}}>
                    <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:18,color:C.text}}>{e.weight}<span style={{fontSize:10,fontWeight:400}}>kg</span></div>
                    {e.waist&&<div style={{fontSize:11,color:C.blue}}>{e.waist}cm waist</div>}
                  </div>
            ...
            {entries.length>14&&(
              <div style={{padding:"10px 14px",borderTop:"1px solid #efe9e1",textAlign:"center"}}>
      ...
      {entries.length===0&&(
        <div style={{textAlign:"center",padding:"40px 0",color:C.textMute}}>
          ...
          <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:18,fontWeight:700,color:C.lineStrong,marginBottom:6}}>No entries yet</div>
...
function SessionLogger({day,dayIdx,plan,week,logs,pbs,exMap,onLog,onBack}){
  ...
  day.exercises.forEach(ex=>{
    ...
    const prevSetsForEx=getPrevForExercise(ex.exerciseId);
    prevByExercise[ex.exerciseId]=prevSetsForEx;
  ...
  const confirmSwap=(newId)=>{
    ...
    if(!sets[newId]){
      ...
      newSets={...sets,[newId]:Array(total).fill(null).map(()=>({weight:"",reps:""}))};
      setSets(newSets);
    }
  ...
  return(<div style={{...base,position:"relative"}}>
    <div style={{padding:"12px 14px",borderBottom:"1px solid #e4ded4",background:C.surfaceAlt,flexShrink:0}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
...
function HomeTab({session,activePlan,currentWeek,logs,pbs,exMap,weightData,calProfile,onGoToToday,onStartSession,userName,onSaveUserName,customMacros,cardioData}){
  ...
  useEffect(()=>{
    ...
    fetch("/api/daily-tip",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
      ...
      lastSteps:lastStepsEntry?{steps:lastStepsEntry.steps,date:lastStepsEntry.date,daysAgo:Math.round((new Date(todayStr)-new Date(lastStepsEntry.date))/86400000)}:null,
  ...
  return(
    <div style={{padding:"14px 14px 32px"}}>
      ...
      <div style={{textAlign:"center",padding:"10px 0 20px"}}>
        ...
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:28,letterSpacing:2,color:C.text}}>THE REEL NEIL</div>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:400,fontSize:14,letterSpacing:4,color:C.textMute,textTransform:"uppercase",marginBottom:10}}>Fitness</div>
        <a href="https://www.instagram.com/the_reel_neil" target="_blank" rel="noopener noreferrer"
      ...
      <div style={{display:"flex",gap:8,marginBottom:12}}>
        {latestWeight&&(
          <div style={{flex:1,...card,padding:"12px",margin:0}}>
            <div style={{fontSize:10,color:C.textMute,fontWeight:700,letterSpacing:0.5,marginBottom:4}}>CURRENT WEIGHT</div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:24,color:C.orange,lineHeight:1}}>{latestWeight.weight}<span style={{fontSize:12,fontWeight:400}}>kg</span></div>
            {toTarget!==null&&<div style={{fontSize:11,color:toTarget<=0?C.green:C.textMute,marginTop:3}}>{toTarget<=0?"Goal reached! 🎉":`${Math.abs(toTarget)}kg to ${target}kg target`}</div>}
          </div>
        )}
        {results&&(
          <div style={{flex:1,...card,padding:"12px",margin:0}}>
            <div style={{fontSize:10,color:C.textMute,fontWeight:700,letterSpacing:0.5,marginBottom:4}}>DAILY CALS</div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:24,color:C.orange,lineHeight:1}}>{results.target.toLocaleString()}</div>
      ...
      {recentPb&&(
        <div style={{...card,padding:"12px 14px",marginBottom:12,display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontWeight:600,fontSize:14,color:C.text}}>{exMap[recentPb[0]]?.name||recentPb[0]}</div>
            <div style={{fontSize:11,color:C.textMute,marginTop:1}}>{new Date(recentPb[1].date).toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"})}</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:22,color:C.orange}}>{recentPb[1].weight}<span style={{fontSize:12,fontWeight:400}}>kg</span></div>
            <div style={{fontSize:11,color:C.textMute}}>{recentPb[1].reps} reps</div>
          </div>
        </div>
      )}
      ...
      <div style={{...card,padding:"14px",marginBottom:4,background:C.orangeBg,border:"1px solid #fae7d8"}}>
        ...
        {!tip&&!tipLoading&&<div style={{fontSize:13,color:C.textMute}}>Set up an active plan to get personalised daily tips.</div>}
      </div>
...
function CardioTab({data,onSave}){
  ...
  return(
    <div style={{padding:"14px 14px 32px"}}>
      <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:22,color:C.text,letterSpacing:0.5,marginBottom:14}}>CARDIO</div>

      {/* Section toggle */}
      <div style={{display:"flex",gap:6,marginBottom:14}}>
        {[["steps","👟 Steps"],["sessions","🏃 Sessions"]].map(([k,l])=>(
          <button key={k} onClick={()=>setActiveSection(k)} style={{flex:1,padding:"10px 8px",borderRadius:8,border:"none",cursor:"pointer",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:13,letterSpacing:0.5,background:activeSection===k?C.orange:C.field,color:activeSection===k?"white":C.textMute}}>
            {l}
          </button>
        ))}
      </div>
        ...
        <>
          ...
          {steps.length>0&&(
            <>
              ...
              <div style={{display:"flex",gap:8,marginBottom:12}}>
                <div style={statBox(C.orange)}>
                  <div style={{fontSize:10,color:C.orange,fontWeight:700,letterSpacing:1,marginBottom:4}}>TODAY / LATEST</div>
                  <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:26,color:C.orange,lineHeight:1}}>{steps[0].steps.toLocaleString()}</div>
                  <div style={{fontSize:10,color:steps[0].steps>=target?C.green:C.textMute,marginTop:3}}>{steps[0].steps>=target?"✓ Target hit!": `${(target-steps[0].steps).toLocaleString()} to go`}</div>
                </div>
                <div style={statBox(C.blue)}>
                  <div style={{fontSize:10,color:C.blue,fontWeight:700,letterSpacing:1,marginBottom:4}}>DAILY AVERAGE</div>
                  <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:26,color:C.blue,lineHeight:1}}>{avgSteps.toLocaleString()}</div>
                  <div style={{fontSize:10,color:C.textMute,marginTop:3}}>target: {target.toLocaleString()}</div>
                </div>
              </div>
              ...
              {steps[0]&&(
                <div style={{...card,padding:"12px 14px",marginBottom:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                    <span style={{fontSize:12,color:C.textMute}}>Today's progress</span>
                    <span style={{fontSize:12,fontWeight:700,color:C.text}}>{Math.min(Math.round((steps[0].steps/target)*100),100)}%</span>
                  </div>
                  <div style={{height:8,background:C.field,borderRadius:4,overflow:"hidden"}}>
                    <div style={{height:"100%",width:`${Math.min((steps[0].steps/target)*100,100)}%`,background:steps[0].steps>=target?C.green:C.orange,borderRadius:4,transition:"width .3s"}}/>
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
                    <span style={{fontSize:10,color:C.textMute}}>0</span>
                    <span style={{fontSize:10,color:C.textMute}}>{target.toLocaleString()} steps</span>
                  </div>
                </div>
              )}
              ...
              {weeklySteps.length>1&&(
                <div style={{...card,padding:"14px 8px 14px 4px",marginBottom:12}}>
                  <div style={{padding:"0 10px 10px",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:13,color:C.textMute,letterSpacing:1}}>WEEKLY AVERAGE STEPS</div>
                  <ResponsiveContainer width="100%" height={170}>
                    <LineChart data={chartData} margin={{top:4,right:16,left:0,bottom:4}}>
                      <CartesianGrid strokeDasharray="3 3" stroke={C.line} vertical={false}/>
                      <XAxis dataKey="week" tick={{fill:C.textMute,fontSize:10}} axisLine={false} tickLine={false}/>
                      <YAxis tick={{fill:C.textMute,fontSize:10}} axisLine={false} tickLine={false} domain={["auto","auto"]} width={44} tickFormatter={v=>v>=1000?`${(v/1000).toFixed(0)}k`:v}/>
                      <ReferenceLine y={target} stroke={C.green} strokeDasharray="4 4" strokeWidth={1.5}/>
                      <Tooltip contentStyle={{background:C.field,border:"1px solid #d8d0c4",borderRadius:8,color:C.text,fontSize:12}} formatter={v=>[v.toLocaleString(),"Avg steps"]}/>
                      <Line type="monotone" dataKey="avg" stroke={C.orange} strokeWidth={2.5} dot={{fill:C.orange,strokeWidth:0,r:3}} activeDot={{r:5}}/>
                    </LineChart>
                  </ResponsiveContainer>
                  <div style={{padding:"0 14px",display:"flex",alignItems:"center",gap:6}}><div style={{width:16,height:2,borderTop:"2px dashed #1aa05f"}}/><span style={{fontSize:10,color:C.green}}>Target {target.toLocaleString()}</span></div>
                </div>
              )}
                ...
                <div style={{marginBottom:12}}>
                  ...
                  <div style={card}>
                    {[...weeklySteps].reverse().map((w,i)=>{
                      ...
                      return(
                        <div key={w.week} style={{display:"flex",alignItems:"center",padding:"10px 14px",borderTop:i===0?"none":"1px solid #efe9e1"}}>
                          <div style={{flex:1}}>
                            <div style={{fontWeight:600,fontSize:13,color:C.text}}>Wk of {fmtDate(w.week)}</div>
                            <div style={{fontSize:11,color:C.textMute,marginTop:1}}>{w.entries} {w.entries===1?"entry":"entries"}</div>
                          </div>
                          <div style={{textAlign:"right"}}>
                            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:18,color:C.orange}}>{w.avg.toLocaleString()}</div>
                            {diff!==null&&<div style={{fontSize:10,color:diff>0?C.green:diff<0?C.red:C.textMute}}>{diff>0?"+":""}{diff.toLocaleString()}</div>}
                          </div>
                        </div>
                      );
                    })}
                ...
                {displayedSteps.map((e,i)=>{
                  ...
                  return(
                    <div key={e.date} style={{display:"flex",alignItems:"center",padding:"9px 14px",borderTop:i===0?"none":"1px solid #efe9e1"}}>
                      <div style={{flex:1}}>
                        <div style={{fontWeight:500,fontSize:14,color:C.text}}>{fmtDate(e.date)}</div>
                        {diff!==null&&<div style={{fontSize:11,marginTop:1,color:diff>0?C.green:diff<0?C.red:C.textMute}}>{diff>0?"+":""}{diff.toLocaleString()}</div>}
                      </div>
                      <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:18,color:e.steps>=target?C.green:C.text,marginRight:12}}>{e.steps.toLocaleString()}</div>
                      <button style={{...btn("danger"),padding:"5px 7px"}} onClick={()=>removeSteps(e.date)}>{Ic.trash}</button>
                    </div>
                  );
                })}
                {steps.length>14&&<div style={{padding:"10px 14px",borderTop:"1px solid #efe9e1",textAlign:"center"}}><button style={{...btn("ghost"),padding:"6px 16px",fontSize:12}} onClick={()=>setShowAllSteps(!showAllSteps)}>{showAllSteps?"Show Less":`Show All ${steps.length}`}</button></div>}
              </div>
            </>
          )}
          {steps.length===0&&<div style={{textAlign:"center",padding:"40px 0",color:C.textMute}}><div style={{fontSize:40,marginBottom:10}}>👟</div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:18,fontWeight:700,color:C.lineStrong,marginBottom:6}}>No steps logged yet</div><div style={{fontSize:13}}>Log your daily steps above to start tracking.</div></div>}
        </>
      )}
          ...
          {sessions.length>0&&(
            <>
              ...
              <div style={{display:"flex",gap:8,marginBottom:12}}>
                ...
                <div style={statBox(C.blue)}>
                  <div style={{fontSize:10,color:C.redBg,fontWeight:700,letterSpacing:1,marginBottom:4}}>TOTAL CALS</div>
