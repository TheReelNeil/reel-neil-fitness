...
function MainApp({session,onLogout}){
  ...
  useEffect(()=>{(async()=>{
    const [ex,pl,ap,cw,lg,pb,cp,wd,un,cm,cd,rd,dl,ms,dp,rm,hk,hm]=await Promise.all([dbLoad(token,"exercises"),dbLoad(token,"plans"),dbLoad(token,"active_plan"),dbLoad(token,"current_week"),dbLoad(token,"logs"),dbLoad(token,"pbs"),dbLoad(token,"cal_profile"),dbLoad(token,"weight_data"),dbLoad(token,"user_name"),dbLoad(token,"custom_macros"),dbLoad(token,"cardio_data"),dbLoad(token,"run_data"),dbLoad(token,"daily_log"),dbLoad(token,"measurements"),dbLoad(token,"daily_prefs"),dbLoad(token,"refeed_macros"),dbLoad(token,"health_sync_token"),dbLoad(token,"health_sync_meta")]);
...
function DailyTab({dailyLog,onSaveDailyLog,weightData,onSaveWeightData,cardioData,onSaveCardioData,measurements,onSaveMeasurements,prefs,onSavePrefs,refeedMacros,onSaveRefeedMacros,calProfile,customMacros,healthToken,onSaveHealthToken,healthMeta}){
  ...
  const save=()=>{
    ...
    ["kcal","protein","carbs","fat","cardioKcals","fluids","sleepHrs","sleepScore","hrv","rhr","cycleDay"].forEach(k=>{if(e[k]==null&&prev[k]!=null)e[k]=prev[k];});
