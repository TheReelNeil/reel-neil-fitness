...
function MainApp({session,onLogout}){
  ...
  useEffect(()=>{(async()=>{
    const [ex,pl,ap,cw,lg,pb,cp,wd,un]=await Promise.all([dbLoad(token,"exercises"),dbLoad(token,"plans"),dbLoad(token,"active_plan"),dbLoad(token,"current_week"),dbLoad(token,"logs"),dbLoad(token,"pbs"),dbLoad(token,"cal_profile"),dbLoad(token,"weight_data"),dbLoad(token,"user_name")]);
    ...
    setCalProfile(cp||null);setWeightData(wd||{entries:[],targetWeight:null});setUserName(un||"");setReady(true);
  })();},[token]);
  ...
  const saveUserName =useCallback(async n=>{setUserName(n);      await sf("user_name",n);},[sf]);
      ...
      <div style={{...card,padding:"12px 14px",marginBottom:12,background:"#2e1808",border:"1px solid #7a2e00"}}>
        ...
        {editingName?(
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            ...
            <button style={{...btn("primary"),padding:"8px 12px"}} onClick={()=>{onSaveUserName(nameInput);setEditingName(false);}}>{Ic.check}</button>
