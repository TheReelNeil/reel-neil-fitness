...
function MainApp({session,onLogout}){
  ...
  useEffect(()=>{(async()=>{
    const [ex,pl,ap,cw,lg,pb,cp,wd,un,cm,cd]=await Promise.all([dbLoad(token,"exercises"),dbLoad(token,"plans"),dbLoad(token,"active_plan"),dbLoad(token,"current_week"),dbLoad(token,"logs"),dbLoad(token,"pbs"),dbLoad(token,"cal_profile"),dbLoad(token,"weight_data"),dbLoad(token,"user_name"),dbLoad(token,"custom_macros"),dbLoad(token,"cardio_data")]);
  ...
  const TABS=[["home","HOME",Ic.home],["today","TRAIN",Ic.today],["plans","PLANS",Ic.plan],["exercises","GYM",Ic.dumbbell],["pbs","PBs",Ic.trophy],["calories","CALS",Ic.calories],["weight","WEIGHT",Ic.scale],["cardio","CARDIO",Ic.cardio]];
...
function HomeTab({session,activePlan,currentWeek,logs,pbs,exMap,weightData,calProfile,onGoToToday,onStartSession,userName,onSaveUserName,customMacros,cardioData}){
  ...
  const thisWeekVol=Object.entries(planLogs[currentWeek]||{}).filter(([k])=>!isNaN(k)).reduce((t,[,day])=>t+Object.values(day).filter(Array.isArray).reduce((s,sets)=>s+sets.reduce((a,st)=>a+(parseFloat(st.weight)||0)*(parseInt(st.reps)||0),0),0),0);
  const lastWeekVol=Object.entries(planLogs[currentWeek-1]||{}).filter(([k])=>!isNaN(k)).reduce((t,[,day])=>t+Object.values(day).filter(Array.isArray).reduce((s,sets)=>s+sets.reduce((a,st)=>a+(parseFloat(st.weight)||0)*(parseInt(st.reps)||0),0),0),0);
          ...
          <div style={{...card,padding:14,marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:8}}>
              <div style={{flex:1}}>
                ...
                <input style={{...inp,height:44,boxSizing:"border-box"}} type="number" inputMode="numeric" placeholder="e.g. 8500" value={stepsInput} onChange={e=>setStepsInput(e.target.value)} onFocus={e=>e.target.select()}/>
              </div>
              <div style={{flex:1}}>
                <label style={lbl}>Date</label>
                <input style={{...inp,height:44,boxSizing:"border-box"}} type="date" value={stepsDate} onChange={e=>setStepsDate(e.target.value)}/>
              </div>
            </div>
        ...
        <>
          <div style={{...card,padding:14,marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:8}}>
              ...
              <div style={{flex:1}}>
                <label style={lbl}>Date</label>
                <input style={inp} type="date" value={sessDate} onChange={e=>setSessDate(e.target.value)}/>
              </div>
            </div>
