...
function MainApp({session,onLogout}){
  ...
  const TABS=[["home","HOME",Ic.home],["today","TRAIN",Ic.today],["plans","PLANS",Ic.plan],["exercises","GYM",Ic.dumbbell],["pbs","PBs",Ic.trophy],["calories","CALS",Ic.calories],["daily","DAILY",Ic.daily],["pics","PICS",Ic.camera],["cardio","CARDIO",Ic.cardio],["running","RUN",Ic.run]];
...
function PicsTab({token,userId,pics,onSavePics}){
  ...
  const pickFile=pose=>{pendingPose.current=pose;setErr("");fileRef.current?.click();};
          ...
          {entries.map(e=>(
            <div key={e.date} style={{...card,padding:"10px 12px",marginBottom:8}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                ...
                <button style={{...btn("ghost"),padding:"4px 10px",fontSize:10}} onClick={()=>{setDate(e.date);window.scrollTo?.(0,0);}}>Open</button>
              </div>
              <div style={{display:"flex",gap:6}}>
                {POSES.map(([key,label])=>{
                  const p=e.poses?.[key];
