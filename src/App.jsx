      ...
      <div style={{padding:"12px 16px 10px",borderBottom:"1px solid #1e1e24",background:"#212126",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            ...
            <div style={{fontSize:10,color:"#505058",marginTop:1}}>{session.email}</div>
          </div>
  ...
  return(
    <div style={{padding:"14px 14px 32px"}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:20}}>
        {[["PROTEIN",results.protein,"g","#f472b6","#3a1a2a"],["CARBS",results.carbs,"g","#34d399","#2a1408"],["FAT",results.fat,"g","#fb923c","#2a1a0a"]].map(([n,v,u,c,bg])=>(
          <div key={n} style={{flex:1,background:bg,border:`1px solid ${c}33`,borderRadius:8,padding:"12px",textAlign:"center"}}>
            ...
            <div style={{fontSize:10,color:"#606068",marginTop:2}}>{u} / day</div>
            <div style={{fontSize:10,color:"#505058",marginTop:1}}>{n==="PROTEIN"?`${v*4}`:`${n==="CARBS"?v*4:v*9}`} kcal</div>
          </div>
        ))}
      </div>
        ...
        return(<div key={ex.exerciseId} style={{...card,margin:"10px 12px"}}>
          <div style={{padding:"12px 14px 8px"}}>
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8}}>
              <div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:18,color:"#f0ece6"}}>{e.name}</div><MuscleChip muscle={e.muscle}/></div>
      ...
      {day.exercises.map((ex,ei)=>{const e=exMap[ex.exerciseId];return(<div key={ei} style={{borderBottom:"1px solid #141418"}}>
        ...
        {ex.setConfigs.map((cfg,ci)=>(<div key={ci} style={{padding:"4px 12px 8px",display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
          {[["SETS","number",cfg.sets,v=>updCfg(di,ei,ci,"sets",parseInt(v)||1),"3"],["REPS","text",cfg.repsRange,v=>updCfg(di,ei,ci,"repsRange",v),"8-12"],["NOTE","text",cfg.note,v=>updCfg(di,ei,ci,"note",v),"e.g. Drop"]].map(([l,t,v,fn,ph])=>(
            <div key={l}><div style={{fontSize:9,color:"#606068",fontWeight:700,letterSpacing:0.5,marginBottom:3}}>{l}</div><input type={t} style={{...setInp,width:"100%",fontSize:t==="text"?12:16}} value={v} onChange={e=>fn(e.target.value)} placeholder={ph}/></div>
          ))}
        </div>))}
      </div>);})}
            ...
            return(
              <div key={ei} style={{borderTop:ei===0?"none":"1px solid #141418"}}>
                <div style={{padding:"9px 14px",display:"flex",alignItems:"center",gap:10}}>
                  ...
                  {e&&<button style={{background:"#2e1808",border:"1px solid #4c1d95",borderRadius:6,padding:"5px 8px",cursor:"pointer",color:"#f0a060",fontSize:12,fontWeight:700,flexShrink:0}} onClick={()=>openSwap(di,ei,ex)}>⇄</button>}
                </div>
                {cfg&&(
