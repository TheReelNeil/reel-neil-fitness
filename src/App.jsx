...
const dbLoad = async (t,k) => { try { const r=await fetch(`${SB_URL}/rest/v1/user_data?key=eq.${k}&select=value`,{headers:dHdr(t)}); const d=await r.json(); return d?.[0]?.value??null; } catch { return null; } };
const dbSave = async (t,uid,k,v) => { try { await fetch(`${SB_URL}/rest/v1/user_data`,{method:"POST",headers:{...dHdr(t),Prefer:"resolution=merge-duplicates"},body:JSON.stringify({user_id:uid,key:k,value:v})}); } catch {} };
...
const MC = {Chest:"#ff6b35",Back:"#4ecdc4",Shoulders:"#a78bfa",Biceps:"#f472b6",Triceps:"#fb923c",Quads:"#34d399",Hamstrings:"#60a5fa",Glutes:"#f59e0b",Adductors:"#e879f9",Calves:"#94a3b8",Core:"#fbbf24",Traps:"#38bdf8",Forearms:"#d97706"};
...
const Ic = {
  ...
  trophy:<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{width:14,height:14}}><polyline points="8 22 12 18 16 22"/><path d="M5 2h14l-2 9H7L5 2z"/><path d="M5 7H3a2 2 0 0 0 0 4h2"/><path d="M19 7h2a2 2 0 0 0 0 4h-2"/><line x1="12" y1="18" x2="12" y2="15"/></svg>,
...
body{background:#0a0a0c;color:#e8e4dc;font-family:'Barlow',sans-serif;font-size:15px;line-height:1.4;overscroll-behavior:none;}
...
const base={display:"flex",flexDirection:"column",height:"100dvh",maxWidth:480,margin:"0 auto",background:"#0a0a0c",overflow:"hidden"};
const inp={background:"#1a1a20",border:"1px solid #2a2a32",borderRadius:7,padding:"11px 14px",color:"#e8e4dc",width:"100%",fontSize:15};
const setInp={background:"#1a1a20",border:"1px solid #2a2a32",borderRadius:6,padding:"8px 6px",color:"#e8e4dc",textAlign:"center",width:"100%",fontSize:16};
const card={background:"#121216",border:"1px solid #1e1e24",borderRadius:10,marginBottom:10,overflow:"hidden"};
const btn=(v="primary")=>({display:"inline-flex",alignItems:"center",gap:6,padding:"10px 16px",borderRadius:7,border:"none",cursor:"pointer",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:13,letterSpacing:0.5,textTransform:"uppercase",
  ...(v==="primary"?{background:"#d4ff3f",color:"#0a0a0c"}:v==="ghost"?{background:"transparent",color:"#66666e",border:"1px solid #2a2a32"}:v==="danger"?{background:"transparent",color:"#ff5555",border:"1px solid #3a2020"}:{background:"#1e1e26",color:"#e8e4dc"})});
const lbl={fontSize:11,fontWeight:700,letterSpacing:1,textTransform:"uppercase",color:"#55555e",display:"block",marginBottom:5};
const statBox=(color="#d4ff3f")=>({background:"#0f0f12",border:`1px solid ${color}22`,borderRadius:8,padding:"12px 14px",flex:1});

function MuscleChip({muscle}){return <span style={{fontSize:10,background:(MC[muscle]||"#55555e")+"22",color:MC[muscle]||"#55555e",padding:"2px 6px",borderRadius:3,fontWeight:700,letterSpacing:0.5,textTransform:"uppercase"}}>{muscle}</span>;}
function MuscleFilter({active,onChange}){return(<div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4,marginBottom:12}}>{["All",...MUSCLES].map(m=><button key={m} onClick={()=>onChange(m)} style={{flexShrink:0,padding:"5px 10px",borderRadius:5,border:"1px solid",fontSize:11,fontWeight:700,letterSpacing:0.5,cursor:"pointer",fontFamily:"'Barlow Condensed',sans-serif",textTransform:"uppercase",background:active===m?(MC[m]||"#d4ff3f"):"transparent",color:active===m?"#0a0a0c":"#55555e",borderColor:active===m?(MC[m]||"#d4ff3f"):"#2a2a30"}}>{m}</button>)}</div>);}
function Divider(){return <div style={{height:1,background:"#1a1a20",margin:"4px 0"}}/>;}
function SectionHead({label}){return <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:11,letterSpacing:1.5,color:"#44444e",textTransform:"uppercase",marginBottom:10,marginTop:4}}>{label}</div>;}
...
export default function App(){
  ...
  if(booting)return(<><style>{css}</style><div style={{...base,alignItems:"center",justifyContent:"center"}}><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:28,fontWeight:800,letterSpacing:3,color:"#d4ff3f"}}>LOADING…</div></div></>);
    ...
    <div style={{...base,alignItems:"center",justifyContent:"center",padding:"0 24px"}}>
      <div style={{width:"100%",maxWidth:360,animation:"fadeIn .4s ease"}}>
        <div style={{textAlign:"center",marginBottom:36}}>
          <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:34,letterSpacing:2,color:"#d4ff3f",lineHeight:1}}>THE REEL NEIL</div>
          <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:400,fontSize:16,letterSpacing:5,color:"#44444e",textTransform:"uppercase",marginTop:4}}>Fitness</div>
...
function MainApp({session,onLogout}){
  ...
  useEffect(()=>{(async()=>{
    const [ex,pl,ap,cw,lg,pb,cp,wd]=await Promise.all([dbLoad(token,"exercises"),dbLoad(token,"plans"),dbLoad(token,"active_plan"),dbLoad(token,"current_week"),dbLoad(token,"logs"),dbLoad(token,"pbs"),dbLoad(token,"cal_profile"),dbLoad(token,"weight_data")]);
    setExercises(ex||DEFAULT_EXERCISES);setPlans(pl||[]);setActivePlanId(ap||null);setCurrentWeek(cw||1);setLogs(lg||{});setPbs(pb||{});
    setCalProfile(cp||null);setWeightData(wd||{entries:[],targetWeight:null});setReady(true);
  })();},[token]);

  const sf=useCallback(async(k,v)=>{setSyncing(true);await dbSave(token,userId,k,v);setSyncing(false);},[token,userId]);
  const savePlans    =useCallback(async p=>{setPlans(p);         await sf("plans",p);},[sf]);
  ...
  if(!ready)return(<><style>{css}</style><div style={{...base,alignItems:"center",justifyContent:"center",gap:8}}><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:800,letterSpacing:2,color:"#d4ff3f"}}>SYNCING…</div><div style={{fontSize:12,color:"#33333a"}}>Loading your data</div></div></>);
  ...
  const TABS=[["today","TODAY",Ic.today],["plans","PLANS",Ic.plan],["exercises","GYM",Ic.dumbbell],["pbs","PBs",Ic.trophy],["calories","CALS",Ic.calories],["weight","WEIGHT",Ic.scale]];

  return(<><style>{css}</style>
    <div style={base}>
      <div style={{padding:"12px 16px 10px",borderBottom:"1px solid #1e1e24",background:"#0d0d10",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:18,letterSpacing:1,color:"#e8e4dc",textTransform:"uppercase"}}>The Reel Neil Fitness</div>
            <div style={{fontSize:10,color:"#33333a",marginTop:1}}>{session.email}</div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            {syncing&&<div style={{fontSize:10,color:"#55555e",fontWeight:600,letterSpacing:0.5}}>SAVING…</div>}
            {activePlan&&<div style={{background:"#d4ff3f",color:"#0a0a0c",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:11,padding:"3px 8px",borderRadius:3,letterSpacing:1}}>WK {currentWeek}</div>}
      ...
      <div style={{flex:1,overflowY:"auto",overflowX:"hidden"}}>
        ...
        {tab==="calories" &&<CaloriesTab profile={calProfile} onSave={saveCalProfile}/>}
        {tab==="weight"   &&<WeightTab data={weightData} onSave={saveWeightData}/>}
      </div>
      <nav style={{display:"flex",borderTop:"1px solid #1e1e24",background:"#0d0d10",flexShrink:0,overflowX:"auto"}}>
        {TABS.map(([id,label,icon])=>(
          <button key={id} style={{flex:"0 0 auto",minWidth:0,width:"16.666%",display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"8px 2px 10px",border:"none",background:"none",color:tab===id?"#d4ff3f":"#55555e",cursor:"pointer",fontSize:9,fontFamily:"'Barlow',sans-serif",fontWeight:600,letterSpacing:0.3,textTransform:"uppercase"}} onClick={()=>setTab(id)}>
            {icon}{label}
          </button>
        ))}
      </nav>
...
function CaloriesTab({profile,onSave}){
  ...
  if(editing||!profile)return(
    <div style={{padding:"14px 14px 32px"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:22,color:"#e8e4dc",letterSpacing:0.5}}>CALORIE CALCULATOR</div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        <div style={{display:"flex",gap:10}}>
          ...
          <div style={{flex:1}}><label style={lbl}>Gender</label>
            <div style={{display:"flex",gap:6}}>
              {["male","female"].map(g=><button key={g} onClick={()=>f("gender",g)} style={{flex:1,padding:"10px",border:"1px solid",borderRadius:7,cursor:"pointer",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:13,textTransform:"uppercase",background:form.gender===g?"#d4ff3f":"#1a1a20",color:form.gender===g?"#0a0a0c":"#55555e",borderColor:form.gender===g?"#d4ff3f":"#2a2a32"}}>{g}</button>)}
            </div>
          </div>
        </div>
        <div style={{display:"flex",gap:10}}>
        ...
        <div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {ACTIVITY_LEVELS.map(a=>(
              <button key={a.key} onClick={()=>f("activityLevel",a.key)} style={{padding:"10px 14px",border:"1px solid",borderRadius:8,cursor:"pointer",textAlign:"left",background:form.activityLevel===a.key?"#1e2800":"#1a1a20",borderColor:form.activityLevel===a.key?"#5a8a00":"#2a2a32"}}>
                <div style={{fontWeight:600,fontSize:14,color:form.activityLevel===a.key?"#d4ff3f":"#e8e4dc"}}>{a.label}</div>
                <div style={{fontSize:11,color:"#44444e",marginTop:2}}>{a.desc}</div>
              </button>
            ))}
          </div>
        </div>
        <div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {GOALS.map(g=>(
              <button key={g.key} onClick={()=>f("goal",g.key)} style={{padding:"10px 14px",border:"1px solid",borderRadius:8,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:10,background:form.goal===g.key?"#1e2800":"#1a1a20",borderColor:form.goal===g.key?"#5a8a00":"#2a2a32"}}>
                <span style={{fontSize:20}}>{g.icon}</span>
                <div>
                  <div style={{fontWeight:600,fontSize:14,color:form.goal===g.key?"#d4ff3f":"#e8e4dc"}}>{g.label}</div>
                  <div style={{fontSize:11,color:"#44444e",marginTop:1}}>{g.desc}</div>
                </div>
              </button>
            ))}
          </div>
  ...
  const adjColor=results.goalAdj>0?"#34d399":results.goalAdj<0?"#f87171":"#d4ff3f";
  ...
  return(
    <div style={{padding:"14px 14px 32px"}}>
      ...
      <div style={{fontSize:11,color:"#44444e",marginBottom:20}}>{profile.age}yr · {profile.gender} · {profile.height}cm · {profile.weight}kg · {act.label}</div>
      ...
      <div style={{background:"linear-gradient(135deg,#1a2800,#0f1a00)",border:"1px solid #3a5a00",borderRadius:12,padding:"20px",marginBottom:14,textAlign:"center"}}>
        <div style={{fontSize:11,fontWeight:700,letterSpacing:1.5,color:"#88aa44",textTransform:"uppercase",marginBottom:6}}>Daily Calorie Target</div>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:56,color:"#d4ff3f",lineHeight:1}}>{results.target.toLocaleString()}</div>
        <div style={{fontSize:12,color:"#88aa44",marginTop:4}}>kcal / day</div>
        <div style={{marginTop:10,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
          <span style={{fontSize:18}}>{goal.icon}</span>
          <span style={{fontSize:13,color:"#e8e4dc",fontWeight:500}}>{goal.label}</span>
      ...
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        <div style={statBox("#a78bfa")}><div style={{fontSize:10,color:"#88668e",fontWeight:700,letterSpacing:1,marginBottom:4}}>BMR</div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:22,color:"#a78bfa"}}>{results.bmr.toLocaleString()}</div><div style={{fontSize:10,color:"#44444e"}}>kcal at rest</div></div>
        <div style={statBox("#60a5fa")}><div style={{fontSize:10,color:"#446688",fontWeight:700,letterSpacing:1,marginBottom:4}}>TDEE</div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:22,color:"#60a5fa"}}>{results.tdee.toLocaleString()}</div><div style={{fontSize:10,color:"#44444e"}}>maintenance</div></div>
      </div>

      {/* Macros */}
      <SectionHead label="Daily Macros"/>
      <div style={{display:"flex",gap:8,marginBottom:20}}>
        {[["PROTEIN",results.protein,"g","#f472b6","#3a1a2a"],["CARBS",results.carbs,"g","#34d399","#0a2a1a"],["FAT",results.fat,"g","#fb923c","#2a1a0a"]].map(([n,v,u,c,bg])=>(
          <div key={n} style={{flex:1,background:bg,border:`1px solid ${c}33`,borderRadius:8,padding:"12px",textAlign:"center"}}>
            <div style={{fontSize:9,fontWeight:700,letterSpacing:1,color:c,marginBottom:6,textTransform:"uppercase"}}>{n}</div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:28,color:c,lineHeight:1}}>{v}</div>
            <div style={{fontSize:10,color:"#44444e",marginTop:2}}>{u} / day</div>
            <div style={{fontSize:10,color:"#33333a",marginTop:1}}>{n==="PROTEIN"?`${v*4}`:`${n==="CARBS"?v*4:v*9}`} kcal</div>
          </div>
        ))}
      </div>

      <div style={{...card,padding:14,background:"#0f0f12"}}>
        <div style={{fontSize:12,color:"#44444e",lineHeight:1.7}}>
          These figures are calculated using the <span style={{color:"#88888e"}}>Mifflin-St Jeor</span> equation and are a starting point — adjust based on how your body responds over 2–3 weeks. Weigh yourself daily and track averages in the Weight tab.
        </div>
      </div>
    </div>
  );
...
function WeightTab({data,onSave}){
  ...
  const [targetInput,setTargetInput]=useState(data?.targetWeight?.toString()||"");
  ...
  const weeklyAvgs=useMemo(()=>{
    ...
    return Object.entries(weeks).sort(([a],[b])=>a.localeCompare(b)).map(([wk,ws])=>({week:wk,avg:Math.round((ws.reduce((s,w)=>s+w,0)/ws.length)*10)/10,entries:ws.length}));
  },[sortedAsc]);

  // chart data - weekly averages
  const chartData=weeklyAvgs.map((w,i)=>({name:`Wk ${i+1}`,avg:w.avg,week:fmtDateShort(w.week)}));

  const addEntry=()=>{
    const w=parseFloat(weightInput);
    if(!w||!dateInput)return;
    const existing=(data?.entries||[]).filter(e=>e.date!==dateInput);
    const newEntries=[...existing,{date:dateInput,weight:w}];
  ...
  const removeEntry=(date)=>{
    onSave({...data,entries:(data?.entries||[]).filter(e=>e.date!==date)});
  };

  const saveTarget=()=>{
    const t=parseFloat(targetInput);
    onSave({...data,targetWeight:t||null});
  };
  ...
  return(
    <div style={{padding:"14px 14px 32px"}}>
      <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:22,color:"#e8e4dc",letterSpacing:0.5,marginBottom:16}}>WEIGHT LOG</div>

      {/* Log entry */}
      <div style={{...card,padding:14,marginBottom:14}}>
        <SectionHead label="Log Today's Weight"/>
        <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
          <div style={{flex:1}}>
            <label style={lbl}>Weight (kg)</label>
            <input style={inp} type="number" step="0.1" placeholder="e.g. 85.4" value={weightInput} onChange={e=>setWeightInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addEntry()}/>
          </div>
          <div style={{flex:1}}>
            <label style={lbl}>Date</label>
            <input style={inp} type="date" value={dateInput} onChange={e=>setDateInput(e.target.value)}/>
          </div>
          <button style={{...btn("primary"),padding:"11px 14px",flexShrink:0}} onClick={addEntry}>{Ic.plus}</button>
        </div>
      </div>
      ...
      {entries.length>0&&(
        <>
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            <div style={statBox("#d4ff3f")}>
              <div style={{fontSize:10,color:"#88aa44",fontWeight:700,letterSpacing:1,marginBottom:4}}>CURRENT</div>
              <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:28,color:"#d4ff3f",lineHeight:1}}>{currentWeight}<span style={{fontSize:13,fontWeight:500}}>kg</span></div>
              <div style={{fontSize:10,color:"#44444e",marginTop:2}}>{fmtDate(latest.date)}</div>
            </div>
            {totalChange!==null&&<div style={statBox(totalChange<0?"#34d399":totalChange>0?"#f87171":"#60a5fa")}>
              <div style={{fontSize:10,color:"#446688",fontWeight:700,letterSpacing:1,marginBottom:4}}>TOTAL CHANGE</div>
              <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:28,color:totalChange<0?"#34d399":totalChange>0?"#f87171":"#60a5fa",lineHeight:1}}>{totalChange>0?"+":""}{totalChange}<span style={{fontSize:13,fontWeight:500}}>kg</span></div>
              <div style={{fontSize:10,color:"#44444e",marginTop:2}}>from {startWeight}kg start</div>
            </div>}
          </div>

          {/* Target */}
          <div style={{...card,padding:14,marginBottom:14}}>
            <SectionHead label="Target Weight"/>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <input style={{...inp,flex:1}} type="number" step="0.1" placeholder="e.g. 80.0" value={targetInput} onChange={e=>setTargetInput(e.target.value)}/>
              <button style={{...btn("primary"),padding:"10px 14px",flexShrink:0}} onClick={saveTarget}>{Ic.check}</button>
            </div>
            {data?.targetWeight&&toTarget!==null&&(
              <div style={{marginTop:10,padding:"10px 12px",background:Math.abs(toTarget)<1?"#0a2a1a":"#1a1a20",borderRadius:7,border:"1px solid",borderColor:Math.abs(toTarget)<1?"#2a5a2a":"#2a2a32",display:"flex",alignItems:"center",gap:8}}>
                {Ic.target}
                <div>
                  <span style={{fontWeight:600,color:toTarget<=0?"#34d399":"#e8e4dc"}}>{data.targetWeight}kg target</span>
                  <span style={{color:"#44444e",fontSize:13}}> — {toTarget<=0?<span style={{color:"#34d399"}}>Goal reached! 🎉</span>:<span>{Math.abs(toTarget)}kg to go</span>}</span>
                </div>
              </div>
            )}
          </div>
          ...
          {weeklyAvgs.length>1&&(
            <div style={{...card,padding:"14px 8px 14px 4px",marginBottom:14}}>
              <div style={{padding:"0 10px 10px",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:13,color:"#55555e",letterSpacing:1}}>WEEKLY AVERAGE TREND</div>
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={chartData} margin={{top:4,right:16,left:0,bottom:4}}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#1e1e24" vertical={false}/>
                  <XAxis dataKey="week" tick={{fill:"#44444e",fontSize:10}} axisLine={false} tickLine={false}/>
                  <YAxis tick={{fill:"#44444e",fontSize:10}} axisLine={false} tickLine={false} domain={["auto","auto"]} width={36}/>
                  {data?.targetWeight&&<ReferenceLine y={data.targetWeight} stroke="#34d399" strokeDasharray="4 4" strokeWidth={1.5}/>}
                  <Tooltip contentStyle={{background:"#1a1a20",border:"1px solid #2a2a32",borderRadius:8,color:"#e8e4dc",fontSize:12}} formatter={(v)=>[`${v}kg`,"Avg"]} labelFormatter={l=>`Week: ${l}`}/>
                  <Line type="monotone" dataKey="avg" stroke="#d4ff3f" strokeWidth={2.5} dot={{fill:"#d4ff3f",strokeWidth:0,r:3}} activeDot={{r:5,fill:"#d4ff3f"}}/>
                </LineChart>
              </ResponsiveContainer>
              {data?.targetWeight&&<div style={{padding:"0 14px",display:"flex",alignItems:"center",gap:6}}><div style={{width:16,height:2,background:"#34d399",borderRadius:1,borderTop:"2px dashed #34d399"}}/><span style={{fontSize:10,color:"#34d399"}}>Target {data.targetWeight}kg</span></div>}
            </div>
          )}
            ...
            <div style={{marginBottom:14}}>
              ...
              <div style={card}>
                {[...weeklyAvgs].reverse().map((w,i)=>{
                  ...
                  const diff=prev?Math.round((w.avg-prev.avg)*10)/10:null;
                  return(
                    <div key={w.week} style={{display:"flex",alignItems:"center",padding:"10px 14px",borderTop:i===0?"none":"1px solid #141418"}}>
                      <div style={{flex:1}}>
                        <div style={{fontWeight:600,fontSize:13,color:"#e8e4dc"}}>Wk of {fmtDate(w.week)}</div>
                        <div style={{fontSize:11,color:"#44444e",marginTop:1}}>{w.entries} {w.entries===1?"entry":"entries"}</div>
                      </div>
                      <div style={{textAlign:"right"}}>
                        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:20,color:"#d4ff3f"}}>{w.avg}<span style={{fontSize:11,fontWeight:400}}>kg</span></div>
                        {diff!==null&&<div style={{fontSize:11,color:diff<0?"#34d399":diff>0?"#f87171":"#44444e"}}>{diff>0?"+":""}{diff}kg</div>}
                      </div>
                    </div>
                  );
                })}
            ...
            {displayed.map((e,i)=>{
              ...
              return(
                <div key={e.date} style={{display:"flex",alignItems:"center",padding:"9px 14px",borderTop:i===0?"none":"1px solid #141418"}}>
                  <div style={{flex:1}}>
                    <div style={{fontWeight:500,fontSize:14,color:"#e8e4dc"}}>{fmtDate(e.date)}</div>
                    {diff!==null&&<div style={{fontSize:11,marginTop:1,color:diff<0?"#34d399":diff>0?"#f87171":"#44444e"}}>{diff>0?"+":""}{diff}kg vs prev</div>}
                  </div>
                  <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:20,color:"#e8e4dc",marginRight:12}}>{e.weight}<span style={{fontSize:11,fontWeight:400}}>kg</span></div>
      ...
      {entries.length===0&&(
        <div style={{textAlign:"center",padding:"40px 0",color:"#33333a"}}>
          <div style={{fontSize:40,marginBottom:10}}>⚖️</div>
          <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:18,fontWeight:700,color:"#2a2a30",marginBottom:6}}>No weigh-ins yet</div>
          <div style={{fontSize:13}}>Log your first weight above to start tracking.</div>
        </div>
      )}
    </div>
  );
...
function TodayTab({activePlan,currentWeek,logs,exMap,onStartSession,onWeekChange,onSelectPlan}){
  if(!activePlan)return(<div style={{padding:32,textAlign:"center"}}><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:32,fontWeight:800,color:"#1e1e26",marginBottom:12}}>NO ACTIVE PLAN</div><div style={{color:"#44444e",marginBottom:24,fontSize:14}}>Create or activate a training plan to get started.</div><button style={{...btn("primary"),width:"100%",justifyContent:"center"}} onClick={onSelectPlan}>{Ic.plus} Create a Plan</button></div>);
  ...
  return(<div style={{padding:"14px 14px 24px"}}>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
      ...
      <div style={{flex:1,textAlign:"center"}}>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:28,color:"#e8e4dc"}}>WEEK {currentWeek}</div>
        {focus&&<div style={{fontSize:11,color:"#d4ff3f",fontWeight:700,letterSpacing:1,textTransform:"uppercase"}}>{focus}</div>}
      </div>
    ...
    {activePlan.days.map((day,i)=>{
      ...
      return(<div key={i} style={{...card,border:dayLog?"1px solid #2a3a10":"1px solid #1e1e24"}}>
        <div style={{padding:"12px 14px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div><div style={{fontSize:10,color:"#55555e",fontWeight:700,letterSpacing:1,textTransform:"uppercase",fontFamily:"'Barlow Condensed',sans-serif"}}>Day {i+1}</div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:20,color:"#e8e4dc",marginTop:2}}>{day.name}</div>
            <div style={{fontSize:12,color:"#44444e",marginTop:2}}>{day.exercises.length} exercises{dayLog&&<span style={{color:"#a0cc20",marginLeft:8}}>· {Math.round(vol).toLocaleString()} kg vol</span>}</div>
          </div>
        ...
        <div style={{padding:"0 14px 12px",display:"flex",flexWrap:"wrap",gap:5}}>
          {day.exercises.slice(0,6).map(ex=>{const e=exMap[ex.exerciseId];return e?<span key={ex.exerciseId} style={{fontSize:11,background:"#1a1a20",border:"1px solid #2a2a30",borderRadius:4,padding:"3px 7px",color:"#88888e"}}>{e.name}</span>:null;})}
          {day.exercises.length>6&&<span style={{fontSize:11,color:"#44444e",padding:"3px 4px"}}>+{day.exercises.length-6} more</span>}
        </div>
      </div>);
    })}
  </div>);
...
function RestTimer(){
  ...
  return(
    <div style={{position:"absolute",bottom:80,right:14,zIndex:100}}>
      {expanded&&(
        <div style={{background:"#0d0d12",border:"1px solid #2a2a32",borderRadius:14,padding:14,marginBottom:8,minWidth:180,boxShadow:"0 8px 32px rgba(0,0,0,0.6)"}}>
          {/* Presets */}
          <div style={{display:"flex",gap:6,marginBottom:12}}>
            {PRESETS.map(p=>(
              <button key={p} onClick={()=>{setDuration(p);stop();}} style={{flex:1,padding:"5px 2px",background:duration===p?"#d4ff3f":"#1a1a20",border:"1px solid",borderColor:duration===p?"#d4ff3f":"#2a2a32",borderRadius:6,color:duration===p?"#0a0a0c":"#88888e",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"'Barlow Condensed',sans-serif"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",marginBottom:12}}>
            <svg width={60} height={60} style={{transform:"rotate(-90deg)"}}>
              <circle cx={30} cy={30} r={r} fill="none" stroke="#1a1a20" strokeWidth={4}/>
              <circle cx={30} cy={30} r={r} fill="none" stroke={done?"#ff5555":running?"#d4ff3f":"#55555e"} strokeWidth={4}
                strokeDasharray={circ} strokeDashoffset={circ*(1-pct)} strokeLinecap="round"
                style={{transition:"stroke-dashoffset 0.9s linear"}}/>
            </svg>
            <div style={{position:"absolute",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:18,color:done?"#ff5555":running?"#d4ff3f":"#e8e4dc"}}>
      ...
      {/* Floating button */}
      <button onClick={()=>setExpanded(e=>!e)} style={{width:52,height:52,borderRadius:"50%",background:running?"#d4ff3f":done?"#ff5555":"#1a1a20",border:`2px solid ${running?"#d4ff3f":done?"#ff5555":"#2a2a32"}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 16px rgba(0,0,0,0.5)",flexDirection:"column",gap:1}}>
        <svg viewBox="0 0 24 24" fill="none" stroke={running?"#0a0a0c":done?"white":"#88888e"} strokeWidth="2" strokeLinecap="round" style={{width:20,height:20}}>
          <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
        </svg>
        {running&&<span style={{fontSize:9,fontWeight:800,color:"#0a0a0c",fontFamily:"'Barlow Condensed',sans-serif",lineHeight:1}}>{fmt(timeLeft)}</span>}
...
function SessionLogger({day,dayIdx,plan,week,logs,pbs,exMap,onLog,onBack}){
  ...
  const handleSave=async()=>{
    ...
    setNewPbIds(tr);
    await onLog({...logs,[plan.id]:{...(logs[plan.id]||{}),[week]:{...(logs[plan.id]?.[week]||{}),[dayIdx]:sets}}},np);
    setSaved(true);
  };
  return(<div style={{...base,position:"relative"}}>
    <div style={{padding:"12px 14px",borderBottom:"1px solid #1e1e24",background:"#0d0d10",flexShrink:0}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <button style={{...btn("ghost"),padding:"8px 10px"}} onClick={onBack}>{Ic.back}</button>
        <div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:18,color:"#e8e4dc"}}>{day.name}</div>
          <div style={{fontSize:11,color:"#55555e",fontFamily:"'Barlow Condensed',sans-serif"}}>{plan.name} · WEEK {week}</div></div>
      </div>
    </div>
    <RestTimer/>
    <div style={{flex:1,overflowY:"auto",overflowX:"hidden"}}>
        ...
        return(<div key={ex.exerciseId} style={{...card,margin:"10px 12px"}}>
          <div style={{padding:"12px 14px 8px"}}>
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8}}>
              <div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:18,color:"#e8e4dc"}}>{e.name}</div><MuscleChip muscle={e.muscle}/></div>
              <div style={{flexShrink:0}}>
                {isNewPb?<span style={{display:"inline-flex",alignItems:"center",gap:4,background:"#d4ff3f",borderRadius:4,padding:"3px 8px",fontSize:11,color:"#0a0a0c",fontWeight:700,animation:"pop .3s ease"}}>{Ic.fire} NEW PB!</span>
                :pb?<span style={{display:"inline-flex",alignItems:"center",gap:4,background:"#1e2800",border:"1px solid #3a5000",borderRadius:4,padding:"3px 8px",fontSize:11,color:"#d4ff3f",fontWeight:600}}>{Ic.trophy} {pb.weight}kg × {pb.reps}</span>
                :<span style={{fontSize:11,color:"#33333a"}}>No PB yet</span>}
              </div>
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"32px 1fr 1fr 56px",gap:6,padding:"4px 12px",background:"#0f0f12"}}>
            {["SET","KG","REPS","PREV"].map(h=><div key={h} style={{fontSize:10,color:"#33333a",fontWeight:700,textAlign:"center"}}>{h}</div>)}
          </div>
          {exSets.map((s,si)=>{const lbl=labels[si],p=prevSets[si];return(
            <div key={si} style={{display:"grid",gridTemplateColumns:"32px 1fr 1fr 56px",gap:6,alignItems:"center",padding:"7px 12px",borderTop:"1px solid #141418",background:s.weight&&s.reps?"#0f1200":"transparent"}}>
              <div style={{textAlign:"center"}}><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:13,fontWeight:700,color:"#55555e"}}>{si+1}</div>{lbl?.reps&&<div style={{fontSize:9,color:"#33333a",fontWeight:600}}>{lbl.reps}</div>}</div>
              <input type="number" min="0" step="0.5" placeholder="kg" value={s.weight} onChange={e=>updateSet(ex.exerciseId,si,"weight",e.target.value)} style={{...setInp,borderColor:s.weight?"#3a4a10":"#2a2a32"}}/>
              <input type="number" min="0" placeholder="reps" value={s.reps} onChange={e=>updateSet(ex.exerciseId,si,"reps",e.target.value)} style={{...setInp,borderColor:s.reps?"#3a4a10":"#2a2a32"}}/>
              <div style={{textAlign:"center",fontSize:12,color:"#44444e"}}>{p?.weight?<><span style={{color:"#e8e4dc",fontWeight:600}}>{p.weight}</span><br/><span style={{fontSize:11}}>{p.reps}r</span></>:"—"}</div>
            </div>);})}
        </div>);
      })}
...
function PlansTab({plans,exercises,activePlanId,exMap,onActivate,onSavePlans}){
  ...
  return(<div style={{padding:"14px 14px 24px"}}>
    ...
    {plans.map(p=>(<div key={p.id} style={{...card,border:p.id===activePlanId?"1px solid #5a8a00":"1px solid #1e1e24"}}>
      <div style={{padding:"12px 14px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>{p.id===activePlanId&&<div style={{fontSize:10,color:"#d4ff3f",fontWeight:700,letterSpacing:1,marginBottom:3}}>✓ ACTIVE</div>}
          <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:20,color:"#e8e4dc"}}>{p.name||"Unnamed Plan"}</div>
          <div style={{fontSize:12,color:"#44444e",marginTop:2}}>{p.days.length} days · {p.weeks} weeks</div>
        </div>
        <div style={{display:"flex",gap:6}}>
      ...
      <div style={{padding:"0 14px 12px",display:"flex",flexWrap:"wrap",gap:5}}>
        {p.days.map((d,i)=><span key={i} style={{fontSize:11,background:"#1a1a20",border:"1px solid #2a2a30",borderRadius:4,padding:"3px 7px",color:"#88888e"}}>{d.name}</span>)}
      </div>
    </div>))}
  </div>);
}
...
function PlanBuilder({plan:initial,exercises,exMap,onSave,onCancel}){
  ...
  return(<div style={{padding:"14px 14px 32px"}}>
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
      <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:16,color:"#e8e4dc",letterSpacing:0.5}}>TRAINING DAYS</div>
    ...
    {plan.days.map((day,di)=>(<div key={di} style={{...card,marginBottom:12}}>
      <div style={{padding:"10px 12px",display:"flex",alignItems:"center",gap:8,borderBottom:"1px solid #1e1e24"}}>
        <div style={{fontSize:11,fontWeight:700,color:"#55555e",fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:1,minWidth:40}}>DAY {di+1}</div>
      ...
      {day.exercises.map((ex,ei)=>{const e=exMap[ex.exerciseId];return(<div key={ei} style={{borderBottom:"1px solid #141418"}}>
        <div style={{padding:"8px 12px",display:"flex",alignItems:"center",gap:8}}>
          <div style={{flex:1}}><div style={{fontWeight:600,fontSize:14,color:"#e8e4dc"}}>{e?.name}</div><MuscleChip muscle={e?.muscle||""}/></div>
        ...
        {ex.setConfigs.map((cfg,ci)=>(<div key={ci} style={{padding:"4px 12px 8px",display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
          {[["SETS","number",cfg.sets,v=>updCfg(di,ei,ci,"sets",parseInt(v)||1),"3"],["REPS","text",cfg.repsRange,v=>updCfg(di,ei,ci,"repsRange",v),"8-12"],["NOTE","text",cfg.note,v=>updCfg(di,ei,ci,"note",v),"e.g. Drop"]].map(([l,t,v,fn,ph])=>(
            <div key={l}><div style={{fontSize:9,color:"#44444e",fontWeight:700,letterSpacing:0.5,marginBottom:3}}>{l}</div><input type={t} style={{...setInp,width:"100%",fontSize:t==="text"?12:16}} value={v} onChange={e=>fn(e.target.value)} placeholder={ph}/></div>
          ))}
        </div>))}
      </div>);})}
...
function ExercisePicker({exercises,existing,onPick,onCancel}){
  ...
  return(<div style={{padding:"14px 14px 24px"}}>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
      ...
      <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:18,color:"#e8e4dc"}}>ADD EXERCISE</div>
    </div>
    <div style={{position:"relative",marginBottom:10}}><div style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:"#44444e"}}>{Ic.search}</div><input style={{...inp,paddingLeft:34}} placeholder="Search…" value={search} onChange={e=>setSearch(e.target.value)}/></div>
    ...
    <div style={card}>
      {filtered.map((e,i)=>{const already=existing.includes(e.id);return(<div key={e.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderTop:i===0?"none":"1px solid #141418",opacity:already?0.4:1,cursor:already?"default":"pointer"}} onClick={()=>!already&&onPick(e.id)}>
        <div style={{width:8,height:8,borderRadius:"50%",background:MC[e.muscle]||"#55555e",flexShrink:0}}/><div style={{flex:1,fontWeight:500,fontSize:15,color:"#e8e4dc"}}>{e.name}</div><MuscleChip muscle={e.muscle}/>{already?<span style={{fontSize:11,color:"#33333a"}}>Added</span>:<span style={{color:"#d4ff3f"}}>{Ic.plus}</span>}
      </div>);})}
      {filtered.length===0&&<div style={{padding:20,textAlign:"center",color:"#33333a",fontSize:13}}>No exercises found</div>}
    </div>
  </div>);
}
...
function ExercisesTab({exercises,onSave}){
  ...
  const filtered=exercises.filter(e=>(muscle==="All"||e.muscle===muscle)&&e.name.toLowerCase().includes(search.toLowerCase()));
  return(<div style={{padding:"14px 14px 24px"}}>
    <div style={{position:"relative",marginBottom:10}}><div style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:"#44444e"}}>{Ic.search}</div><input style={{...inp,paddingLeft:34}} placeholder="Search exercises…" value={search} onChange={e=>setSearch(e.target.value)}/></div>
    <MuscleFilter active={muscle} onChange={setMuscle}/>
    {adding&&(<div style={{...card,padding:14,marginBottom:12}}>
      <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:14,marginBottom:10,color:"#e8e4dc"}}>NEW EXERCISE</div>
    ...
    <div style={card}>
      {filtered.map((e,i)=>(<div key={e.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderTop:i===0?"none":"1px solid #141418"}}><div style={{width:8,height:8,borderRadius:"50%",background:MC[e.muscle]||"#55555e",flexShrink:0}}/><div style={{flex:1,fontWeight:500,fontSize:15,color:"#e8e4dc"}}>{e.name}</div><MuscleChip muscle={e.muscle}/></div>))}
      {filtered.length===0&&<div style={{padding:20,textAlign:"center",color:"#33333a",fontSize:13}}>No exercises found</div>}
    </div>
    <div style={{fontSize:11,color:"#33333a",textAlign:"center",marginTop:10}}>{exercises.length} exercises in library</div>
  </div>);
}
...
function PBsTab({pbs,exMap}){
  ...
  if(entries.length===0)return(<div style={{padding:32,textAlign:"center",color:"#33333a"}}><div style={{fontSize:48,marginBottom:8}}>🏆</div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:20,fontWeight:700,color:"#2a2a30",marginBottom:6}}>NO PBs YET</div><div style={{fontSize:13}}>Start logging sessions and your personal bests will appear here automatically.</div></div>);
  const byMuscle={};entries.forEach(e=>{const m=e.ex?.muscle||"Other";if(!byMuscle[m])byMuscle[m]=[];byMuscle[m].push(e);});
  return(<div style={{padding:"14px 14px 32px"}}>
    <div style={{fontSize:11,fontWeight:700,letterSpacing:1,color:"#55555e",marginBottom:16,fontFamily:"'Barlow Condensed',sans-serif"}}>PERSONAL BESTS · {entries.length} EXERCISES</div>
    {Object.entries(byMuscle).map(([muscle,list])=>(<div key={muscle} style={{marginBottom:14}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}><div style={{width:6,height:6,borderRadius:"50%",background:MC[muscle]||"#55555e"}}/><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:13,color:MC[muscle]||"#55555e",letterSpacing:1,textTransform:"uppercase"}}>{muscle}</div></div>
      <div style={card}>{list.map((e,i)=>{const orm=est1RM(e.weight,e.reps),date=e.date?new Date(e.date).toLocaleDateString("en-GB",{day:"numeric",month:"short"}):"";return(<div key={e.id} style={{padding:"10px 14px",borderTop:i===0?"none":"1px solid #141418",display:"flex",alignItems:"center",gap:10}}>
        <div style={{flex:1}}><div style={{fontWeight:600,fontSize:14,color:"#e8e4dc"}}>{e.ex.name}</div><div style={{fontSize:11,color:"#44444e",marginTop:2}}>{date}</div></div>
        <div style={{textAlign:"right"}}><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:22,color:"#d4ff3f"}}>{e.weight}<span style={{fontSize:12,fontWeight:500}}>kg</span></div><div style={{fontSize:11,color:"#55555e"}}>{e.reps} reps · est {orm}kg 1RM</div></div>
      </div>);})}
      </div>
    </div>))}
  </div>);
    ...
    return (
      <div style={{padding:"14px 14px 32px"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
          ...
          <div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:18,color:"#e8e4dc"}}>SWAP EXERCISE</div>
            <div style={{fontSize:11,color:"#55555e",marginTop:1}}>Replacing: {swapping.name}</div>
          </div>
        </div>
        ...
        {!browsingSwap && (
          <>
            <div style={{fontSize:11,fontWeight:700,letterSpacing:1,color:"#a78bfa",marginBottom:10,textTransform:"uppercase"}}>{"✨"} AI Recommendations</div>
            {suggestLoading ? (
              <div style={{...card,padding:20,textAlign:"center"}}>
                <div style={{fontSize:13,color:"#55555e"}}>Getting recommendations…</div>
              </div>
                ...
                {suggestions.map((s,i) => {
                  ...
                  return e ? (
                    <div key={s.id} style={{padding:"12px 14px",borderTop:i===0?"none":"1px solid #141418",display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={()=>doSwap(s.id)}>
                      <div style={{flex:1}}>
                        <div style={{fontWeight:600,fontSize:14,color:"#e8e4dc"}}>{e.name}</div>
                        <div style={{fontSize:11,color:"#7c6aaa",marginTop:2}}>{s.reason}</div>
                      </div>
                      <MuscleChip muscle={e.muscle}/>
                      <span style={{color:"#d4ff3f",fontSize:18}}>›</span>
                    </div>
                  ) : null;
                })}
              </div>
            ) : (
              <div style={{...card,padding:14,marginBottom:14,textAlign:"center",color:"#44444e",fontSize:13}}>No suggestions available</div>
            )}
        ...
        {browsingSwap && (
          <>
            <div style={{fontSize:11,fontWeight:700,letterSpacing:1,color:"#55555e",marginBottom:10,textTransform:"uppercase"}}>All {swapping.muscle} Exercises</div>
            <div style={card}>
              {muscleExercises.map((e,i) => (
                <div key={e.id} style={{padding:"11px 14px",borderTop:i===0?"none":"1px solid #141418",display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={()=>doSwap(e.id)}>
                  <div style={{flex:1,fontWeight:500,fontSize:14,color:"#e8e4dc"}}>{e.name}</div>
                  <span style={{color:"#d4ff3f",fontSize:18}}>›</span>
                </div>
              ))}
            </div>
    ...
    <div style={{padding:"14px 14px 32px"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
        ...
        <div>
          ...
          <div style={{fontSize:12,color:"#55555e",marginTop:1}}>Tap the swap icon to replace any exercise</div>
        </div>
      </div>

      <div style={{...card,padding:14,marginBottom:14,background:"#110d1e",border:"1px solid #4c1d95"}}>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:22,color:"#e8e4dc"}}>{plan.name}</div>
        <div style={{fontSize:12,color:"#7c3aed",marginTop:2}}>{plan.days?.length} days · {plan.weeks} weeks</div>
      </div>
            ...
            return(
              <div key={ei} style={{borderTop:ei===0?"none":"1px solid #141418"}}>
                <div style={{padding:"9px 14px",display:"flex",alignItems:"center",gap:10}}>
                  ...
                  {e&&<button style={{background:"#1e1530",border:"1px solid #4c1d95",borderRadius:6,padding:"5px 8px",cursor:"pointer",color:"#a78bfa",fontSize:12,fontWeight:700,flexShrink:0}} onClick={()=>openSwap(di,ei,ex)}>⇄</button>}
                </div>
                {cfg&&(
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,padding:"0 14px 10px"}}>
                    <div>
                      <div style={{fontSize:9,color:"#44444e",fontWeight:700,letterSpacing:0.5,marginBottom:3}}>SETS</div>
                      <input type="number" min="1" max="8" value={cfg.sets}
                        onChange={e=>updateSetsReps("sets",parseInt(e.target.value)||1)}
                        style={{...setInp,width:"100%",fontSize:14}}/>
                    </div>
                    <div>
                      <div style={{fontSize:9,color:"#44444e",fontWeight:700,letterSpacing:0.5,marginBottom:3}}>REPS</div>
