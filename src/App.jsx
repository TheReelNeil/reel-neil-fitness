...
function ExercisePicker({exercises,existing,onPick,onCancel}){
  ...
  return(<div style={{padding:"14px 14px 24px"}}>
    ...
    <div style={card}>{filtered.map((e,i)=>{const already=existing.includes(e.id);return(<div key={e.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderTop:i===0?"none":"1px solid #141418",opacity:already?0.4:1,cursor:already?"default":"pointer"}} onClick={()=>!already&&onPick(e.id)}>
    ...
    </div>);}){filtered.length===0&&<div style={{padding:20,textAlign:"center",color:"#33333a",fontSize:13}}>No exercises found</div>}</div>
  </div>);
}
...
function ExercisesTab({exercises,onSave}){
  ...
  return(<div style={{padding:"14px 14px 24px"}}>
    ...
    <div style={card}>{filtered.map((e,i)=>(<div key={e.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderTop:i===0?"none":"1px solid #141418"}}><div style={{width:8,height:8,borderRadius:"50%",background:MC[e.muscle]||"#55555e",flexShrink:0}}/><div style={{flex:1,fontWeight:500,fontSize:15,color:"#e8e4dc"}}>{e.name}</div><MuscleChip muscle={e.muscle}/></div>))}{filtered.length===0&&<div style={{padding:20,textAlign:"center",color:"#33333a",fontSize:13}}>No exercises found</div>}</div>
