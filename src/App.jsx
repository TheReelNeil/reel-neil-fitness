...
<button style={{...btn("ghost"),padding:"6px 9px"}} onClick={onLogout}>{Ic.logout}</button>
      ...
      <div style={{flex:1,overflowY:"auto",overflowX:"hidden"}}>
        {tab==="home"     &&<HomeTab session={session} activePlan={activePlan} currentWeek={currentWeek} logs={logs} pbs={pbs} exMap={exMap} weightData={weightData} calProfile={calProfile} onGoToToday={()=>setTab("today")} onStartSession={setSessionDay}/>}
    ...
    <div style={{padding:"14px 14px 32px"}}>
      ...
      <div style={{...card,padding:14,marginBottom:14}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          ...
          <div style={{flex:1}}>
            <label style={lbl}>Waist (cm) <span style={{color:"#505058",fontWeight:400,fontSize:9,letterSpacing:0}}>optional</span></label>
            <input style={inp} type="number" step="0.1" placeholder="e.g. 86.5" value={waistInput} onChange={e=>setWaistInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addEntry()}/>
          </div>
        </div>
        <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
          ...
          <button style={{...btn("primary"),padding:"11px 20px",flexShrink:0}} onClick={addEntry}>{Ic.plus} Log</button>
        </div>
      </div>
...
// ─── Home Tab ─────────────────────────────────────────────────────────────────
function HomeTab({session,activePlan,currentWeek,logs,pbs,exMap,weightData,calProfile,onGoToToday,onStartSession}){
  ...
  return(
    <div style={{padding:"14px 14px 32px"}}>
      ...
      <div style={{...card,padding:"12px 14px",marginBottom:12,background:"#2e1808",border:"1px solid #7a2e00"}}>
        <div style={{fontSize:11,color:"#c06030",fontWeight:600,letterSpacing:0.5,marginBottom:2}}>WELCOME BACK</div>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:18,color:"#f0ece6"}}>{session.email}</div>
      </div>
