...
body{background:${C.bg};color:${C.text};font-family:'Barlow',sans-serif;font-size:15px;line-height:1.4;overscroll-behavior:none;position:fixed;inset:0;width:100%;overflow:hidden;}
...
// Temporary layout diagnostics (shown under the nav while we chase the iOS bottom-gap bug)
function DebugBar(){
  ...
  useEffect(()=>{
    ...
    const iv=setInterval(u,2500);
    return ()=>{window.removeEventListener("resize",u);window.visualViewport?.removeEventListener("resize",u);clearInterval(iv);};
  },[]);
  const sab=getComputedStyle(document.documentElement).getPropertyValue("--sab").trim()||"?";
  const appH=getComputedStyle(document.documentElement).getPropertyValue("--appH").trim()||"?";
  return(
    <div style={{textAlign:"center",fontSize:9,color:C.textFaint,padding:"1px 0 2px",letterSpacing:0.3}}>
      dbg2: inH {window.innerHeight} · scr {window.screen?.height} · appH {appH} · sat {getComputedStyle(document.documentElement).getPropertyValue("--sat").trim()||"?"} · sab {sab} · sa {window.navigator.standalone===true||window.matchMedia?.("(display-mode: standalone)")?.matches?"y":"n"}
    </div>
  );
}
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const measure=()=>{
      // iOS standalone PWAs draw full-screen but report window/viewport height
      // EXCLUDING the status-bar area (e.g. 793 on an 852pt screen) — add the
      // top safe-area inset back so the shell reaches the real bottom edge.
      const standalone=window.navigator.standalone===true||window.matchMedia?.("(display-mode: standalone)")?.matches;
      const sat=parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--sat"))||0;
      const h=Math.round((window.visualViewport?.height??window.innerHeight)+(standalone?sat:0));
    ...
    return ()=>{window.removeEventListener("resize",measure);window.visualViewport?.removeEventListener("resize",measure);window.removeEventListener("orientationchange",measure);window.removeEventListener("pageshow",measure);window.removeEventListener("focusout",onFocusOut);clearInterval(iv);};
  },[]);
...
<button style={{...btn("ghost"),padding:"6px 9px"}} onClick={onLogout}>{Ic.logout}</button>
      ...
      <nav style={{borderTop:`1px solid ${C.lineStrong}`,background:C.surface,flexShrink:0,boxShadow:"0 -2px 10px rgba(44,44,48,0.07)",paddingBottom:"env(safe-area-inset-bottom)"}}>
