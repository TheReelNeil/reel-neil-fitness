...
function MainApp({session,onLogout}){
  ...
  useEffect(()=>{(async()=>{
    const [ex,pl,ap,cw,lg,pb,cp,wd,un,cm,cd,rd,dl,ms,dp,rm]=await Promise.all([dbLoad(token,"exercises"),dbLoad(token,"plans"),dbLoad(token,"active_plan"),dbLoad(token,"current_week"),dbLoad(token,"logs"),dbLoad(token,"pbs"),dbLoad(token,"cal_profile"),dbLoad(token,"weight_data"),dbLoad(token,"user_name"),dbLoad(token,"custom_macros"),dbLoad(token,"cardio_data"),dbLoad(token,"run_data"),dbLoad(token,"daily_log"),dbLoad(token,"measurements"),dbLoad(token,"daily_prefs"),dbLoad(token,"refeed_macros")]);
  ...
  const TABS=[["home","HOME",Ic.home],["today","TRAIN",Ic.today],["plans","PLANS",Ic.plan],["exercises","GYM",Ic.dumbbell],["pbs","PBs",Ic.trophy],["calories","CALS",Ic.calories],["daily","DAILY",Ic.daily],["cardio","CARDIO",Ic.cardio],["running","RUN",Ic.run]];
...
function DailyTab({dailyLog,onSaveDailyLog,weightData,onSaveWeightData,cardioData,onSaveCardioData,measurements,onSaveMeasurements,prefs,onSavePrefs,refeedMacros,onSaveRefeedMacros,calProfile,customMacros}){
  const [date,setDate]=useState(today());
  const blank={kcal:"",protein:"",carbs:"",fat:"",weight:"",steps:"",cardioKcals:"",fluids:"",sleepHrs:"",sleepScore:"",hrv:"",rhr:"",trained:false,offPlan:false,alcohol:false,refeed:false,cycleDay:"",cycleSymptoms:"",notes:""};
  ...
  const f=(k,v)=>setForm(p=>({...p,[k]:v}));
  const numField=(label,k,ph,step)=>(
    <div style={{flex:1,minWidth:0}}>
      <label style={lbl}>{label}</label>
      <input style={inp} type="number" inputMode="decimal" step={step||"any"} placeholder={ph} value={form[k]} onChange={e=>f(k,e.target.value)} onFocus={e=>e.target.select()}/>
    </div>);
  ...
  const save=()=>{
    const e={kcal:numOrNull(form.kcal),protein:numOrNull(form.protein),carbs:numOrNull(form.carbs),fat:numOrNull(form.fat),cardioKcals:numOrNull(form.cardioKcals),fluids:numOrNull(form.fluids),sleepHrs:numOrNull(form.sleepHrs),sleepScore:numOrNull(form.sleepScore),hrv:numOrNull(form.hrv),rhr:numOrNull(form.rhr),trained:form.trained,offPlan:form.offPlan,alcohol:form.alcohol,refeed:form.refeed,notes:form.notes||"",
  ...
  const bar=(label,val,target,unit,color)=>{
    ...
    const over=v!=null&&v>target*1.02;
    return(
      <div key={label} style={{marginBottom:8}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
          ...
          <span style={{fontSize:11,fontWeight:700,color:v==null?C.textFaint:over?C.red:C.text}}>{v!=null?v.toLocaleString():"—"} / {target.toLocaleString()}{unit}</span>
        </div>
        <div style={{height:7,background:C.field,borderRadius:4,overflow:"hidden"}}>
          <div style={{height:"100%",width:`${Math.min(pct,1)*100}%`,background:over?C.red:v!=null&&pct>=0.98?C.green:color,borderRadius:4,transition:"width .3s"}}/>
        </div>
      </div>);
  };

  // Trends
  const METRICS=[["weight","Weight","kg",C.orange],["steps","Steps","",C.green],["kcal","Calories","kcal",C.blue],["protein","Protein","g",C.gold],["sleepHrs","Sleep","hrs",C.blue],["hrv","HRV","",C.green],["rhr","RHR","bpm",C.red],["waist","Waist","cm",C.blue]];
  const series=useMemo(()=>{
    ...
    if(metric==="waist")return(measurements?.entries||[]).filter(e=>e.waist!=null).map(e=>({date:e.date,value:e.waist}));
  ...
  return(
    <div style={{padding:"14px 14px 32px"}}>
      ...
      <div style={{...card,padding:14,marginBottom:12}}>
        <SectionHead label="Nutrition — from MyFitnessPal"/>
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          {numField("Calories","kcal","kcal")}
        ...
        {targets&&(
          <div style={{background:C.surfaceAlt,borderRadius:8,padding:"10px 12px"}}>
            {bar("Calories",form.kcal,targets.kcal," kcal",C.orange)}
      ...
      {/* Body & activity */}
      <div style={{...card,padding:14,marginBottom:12}}>
        <SectionHead label="Body & Activity"/>
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          {numField("Weight (kg)","weight","e.g. 85.4","0.1")}
          {numField("Steps","steps","e.g. 11000","1")}
        </div>
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          {numField("Cardio (kcal)","cardioKcals","e.g. 300","1")}
          {numField("Fluids (L)","fluids","e.g. 3.0","0.1")}
        </div>
        {stepsTarget&&bar("Steps",form.steps,stepsTarget,"",C.green)}
      </div>

      {/* Recovery */}
      <div style={{...card,padding:14,marginBottom:12}}>
        <SectionHead label="Sleep & Recovery"/>
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          {numField("Sleep (hrs)","sleepHrs","e.g. 7.5","0.1")}
          {numField("Sleep Score","sleepScore","e.g. 82","1")}
        </div>
        <div style={{display:"flex",gap:8}}>
          {numField("HRV","hrv","e.g. 45","1")}
          {numField("RHR (bpm)","rhr","e.g. 58","1")}
        </div>
      </div>
            ...
            {displayedHist.map((d,i)=>{
              ...
              return(
                <div key={d} style={{display:"flex",alignItems:"center",padding:"9px 14px",borderTop:i===0?"none":"1px solid #efe9e1"}}>
                  <div style={{flex:1,cursor:"pointer"}} onClick={()=>{setDate(d);window.scrollTo?.(0,0);}}>
                  ...
                  <button style={{...btn("ghost"),padding:"5px 9px",marginRight:6,fontSize:11}} onClick={()=>{setDate(d);}}>Edit</button>
