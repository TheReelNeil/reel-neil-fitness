...
<button style={{...btn("ghost"),padding:"6px 9px"}} onClick={onLogout}>{Ic.logout}</button>
      ...
      <div style={{flex:1,overflowY:"auto",overflowX:"hidden"}}>
        ...
        {tab==="today"    &&<TodayTab activePlan={activePlan} currentWeek={currentWeek} logs={logs} exMap={exMap} onStartSession={setSessionDay} onWeekChange={saveWeek} onSelectPlan={()=>setTab("plans")} onSaveLogs={saveLogs}/>}
...
// ─── Today Tab ────────────────────────────────────────────────────────────────
function TodayTab({activePlan,currentWeek,logs,exMap,onStartSession,onWeekChange,onSelectPlan,onSaveLogs}){
...
function SessionLogger({day,dayIdx,plan,week,logs,pbs,exMap,onLog,onBack}){
  ...
  const existingNotes=logs?.[plan.id]?.[week]?.[`${dayIdx}_notes`]||"";
