...
const dbSave = async (t,uid,k,v) => {
  const doSave = async (token) => {
    ...
    const upd = await fetch(
      `${SB_URL}/rest/v1/user_data?key=eq.${encodeURIComponent(k)}&user_id=eq.${uid}`,
      { method:"PATCH", headers:{...dHdr(token),"Prefer":"return=minimal"}, body:JSON.stringify({value:v}) }
    );
    if(upd.ok){
      // Check if any row was actually updated
      const count = upd.headers.get("content-range");
      if(count && count !== "*/0") return upd;
      // No row existed — insert instead
      const ins = await fetch(`${SB_URL}/rest/v1/user_data`,
        { method:"POST", headers:{...dHdr(token),"Prefer":"return=minimal"}, body:JSON.stringify({user_id:uid,key:k,value:v}) }
      );
      return ins;
    }
    return upd;
