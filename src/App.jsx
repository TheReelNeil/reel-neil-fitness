...
// Fixed shell pinned to the real screen edges — avoids iOS PWA quirks where
// 100dvh drifts after the keyboard opens (nav floating mid-screen).
const base={display:"flex",flexDirection:"column",position:"fixed",top:0,bottom:0,left:0,right:0,maxWidth:480,margin:"0 auto",background:C.bg,overflow:"hidden"};
...
export default function App(){
  ...
  // iOS standalone-PWA keyboard bug: after the keyboard closes, the viewport can be
  // left scrolled/shrunken, leaving a dead strip under the bottom nav. Snap it back
  // whenever the keyboard goes away or the visual viewport resizes.
  useEffect(()=>{
    const typing=()=>{const t=document.activeElement?.tagName;return t==="INPUT"||t==="TEXTAREA"||t==="SELECT";};
    const restore=()=>{if(typing())return;window.scrollTo(0,0);document.documentElement.scrollTop=0;document.body.scrollTop=0;};
    const onFocusOut=()=>setTimeout(restore,60);
    window.visualViewport?.addEventListener("resize",restore);
    window.addEventListener("focusout",onFocusOut);
    return ()=>{window.visualViewport?.removeEventListener("resize",restore);window.removeEventListener("focusout",onFocusOut);};
  },[]);
