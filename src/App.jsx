...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const measure=()=>{
      ...
      const reported=vv?.height??window.innerHeight;
      const offTop=vv?.offsetTop??0;
      let h=Math.round(reported);
      if(standalone){
        const add=Math.max(0,sat-offTop);           // shift already applied? add less
        const scrH=Math.max(window.screen?.height||0,window.screen?.width||0);
        h=Math.round(Math.min(reported+add,scrH||(reported+add)));
      }
