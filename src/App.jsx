...
html{height:100%;overflow:hidden;background:${C.surface};}
...
const base={display:"flex",flexDirection:"column",position:"fixed",top:"var(--appTop, 0px)",left:0,right:0,height:"var(--appH, 100dvh)",maxWidth:480,margin:"0 auto",background:C.bg,overflow:"hidden"};
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const measure=()=>{
      ...
      const kb=typing();
      // Keyboard open: iOS shifts the visible viewport down by offsetTop. Move the fixed shell to
      // match so the content sits directly above the keyboard instead of leaving a dead grey gap.
      document.documentElement.style.setProperty("--appTop",(kb?Math.round(vv?.offsetTop||0):0)+"px");
      if(!kb){window.scrollTo(0,0);document.documentElement.scrollTop=0;document.body.scrollTop=0;}
    };
    measure();
    const onFocusOut=()=>setTimeout(measure,60);
    const onFocusIn=e=>{const t=e.target;if(t&&(t.tagName==="INPUT"||t.tagName==="TEXTAREA")){setTimeout(()=>{measure();try{t.scrollIntoView({block:"center"});}catch{}},250);}};
...
<button style={{...btn("ghost"),padding:"6px 9px"}} onClick={onLogout}>{Ic.logout}</button>
      ...
      <nav style={{borderTop:`1px solid ${C.lineStrong}`,background:C.surface,flexShrink:0,boxShadow:"0 -2px 10px rgba(44,44,48,0.07)",paddingBottom:6}}>
