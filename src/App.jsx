...
<button style={{...btn("ghost"),padding:"6px 9px"}} onClick={onLogout}>{Ic.logout}</button>
      ...
      <div style={{flex:1,overflowY:"auto",overflowX:"hidden"}}>
        {tab==="home"     &&<HomeTab session={session} activePlan={activePlan} currentWeek={currentWeek} logs={logs} pbs={pbs} exMap={exMap} weightData={weightData} calProfile={calProfile} onGoToToday={()=>setTab("today")} onStartSession={setSessionDay} userName={userName} onSaveUserName={saveUserName} customMacros={customMacros}/>}
...
function TodayTab({activePlan,currentWeek,logs,exMap,onStartSession,onWeekChange,onSelectPlan,onSaveLogs,plans,onSavePlans}){
  ...
  const moveDay=(di,dir)=>{
    if(!activePlan)return;
    const days=[...activePlan.days];
    const j=di+dir;
    if(j<0||j>=days.length)return;
    [days[di],days[j]]=[days[j],days[di]];
    const updated={...activePlan,days};
    onSavePlans(prev=>prev.map(p=>p.id===activePlan.id?updated:p));

    // Swap log entries for both day indices across all weeks
    const planLogs=logs[activePlan.id];
    if(planLogs){
      const updatedLogs={...logs,[activePlan.id]:{}};
      Object.entries(planLogs).forEach(([wk,weekData])=>{
        const newWeek={...weekData};
        // Swap di <-> j for sets and notes
        const tempSets=newWeek[di];
        const tempNotes=newWeek[`${di}_notes`];
        newWeek[di]=newWeek[j];
        newWeek[`${di}_notes`]=newWeek[`${j}_notes`];
        newWeek[j]=tempSets;
        newWeek[`${j}_notes`]=tempNotes;
        // Clean up undefined keys
        if(newWeek[di]===undefined)delete newWeek[di];
        if(newWeek[j]===undefined)delete newWeek[j];
        if(newWeek[`${di}_notes`]===undefined)delete newWeek[`${di}_notes`];
        if(newWeek[`${j}_notes`]===undefined)delete newWeek[`${j}_notes`];
        updatedLogs[activePlan.id][wk]=newWeek;
      });
      onSaveLogs(updatedLogs);
    }
  };
  ...
  return(<div style={{padding:"14px 14px 24px"}}>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
      ...
      <button style={{...btn(reordering?"primary":"ghost"),padding:"8px 10px",fontSize:11,fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,letterSpacing:0.5,textTransform:"uppercase"}} onClick={()=>setReordering(r=>!r)}>
        {reordering?"✓ Done":"⇅"}
      </button>
    </div>
    {reordering&&<div style={{background:"#2e1808",border:"1px solid #7a2e00",borderRadius:8,padding:"9px 14px",marginBottom:10,fontSize:12,color:"#f0a060"}}>
      Tap ▲ ▼ to reorder days. Changes save automatically.
    </div>}
    {activePlan.days.map((day,i)=>{
      const dayLog=logs?.[activePlan.id]?.[currentWeek]?.[i];
      const vol=dayLog?Object.values(dayLog).filter(Array.isArray).reduce((s,sets)=>s+sets.reduce((a,st)=>a+(st.weight*st.reps||0),0),0):0;
          ...
          <div style={{display:"flex",alignItems:"flex-start",gap:8,flex:1}}>
            {reordering&&(
              <div style={{display:"flex",flexDirection:"column",gap:3,flexShrink:0,marginTop:2}}>
                <button style={{background:"#323238",border:"1px solid #44444c",borderRadius:4,width:24,height:22,cursor:"pointer",color:"#f0ece6",fontSize:11,display:"flex",alignItems:"center",justifyContent:"center",opacity:i===0?0.3:1}} onClick={()=>moveDay(i,-1)} disabled={i===0}>▲</button>
                <button style={{background:"#323238",border:"1px solid #44444c",borderRadius:4,width:24,height:22,cursor:"pointer",color:"#f0ece6",fontSize:11,display:"flex",alignItems:"center",justifyContent:"center",opacity:i===activePlan.days.length-1?0.3:1}} onClick={()=>moveDay(i,1)} disabled={i===activePlan.days.length-1}>▼</button>
              </div>
            )}
            <div>
...
// ─── Home Tab ─────────────────────────────────────────────────────────────────
function HomeTab({session,activePlan,currentWeek,logs,pbs,exMap,weightData,calProfile,onGoToToday,onStartSession,userName,onSaveUserName,customMacros}){
  ...
  const thisWeekVol=Object.values(planLogs[currentWeek]||{}).filter(Array.isArray).reduce((t,sets)=>t+sets.reduce((a,st)=>a+(parseFloat(st.weight)||0)*(parseInt(st.reps)||0),0),0);
  const lastWeekVol=Object.values(planLogs[currentWeek-1]||{}).filter(Array.isArray).reduce((t,sets)=>t+sets.reduce((a,st)=>a+(parseFloat(st.weight)||0)*(parseInt(st.reps)||0),0),0);
