...
function MainApp({session,onLogout}){
  ...
  const sf=useCallback(async(k,v)=>{
    ...
    const ok=await dbSave(token,userId,k,v);
    if(!ok){
      setSaveError("⚠️ Save failed — check your connection");
  ...
  const savePlans    =useCallback(async p=>{
    setPlans(prev=>{
      // p can be a new array or a function — always save after React resolves latest state
