...
function MainApp({session,onLogout}){
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              ...
              <div style={{fontSize:10,color:"#72727c",marginTop:1}}>{session.email}</div>
            </div>
          </div>
  ...
  style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:34,height:34,borderRadius:7,border:"1px solid #44444c",background:"transparent",textDecoration:"none",fontSize:16}}>
  {"📸"}
</a>
<button style={{...btn("ghost"),padding:"6px 9px"}} onClick={onLogout}>{Ic.logout}</button>
      ...
      <nav style={{display:"flex",borderTop:"1px solid #1e1e24",background:"#212126",flexShrink:0,overflowX:"auto"}}>
        {TABS.map(([id,label,icon])=>(
          <button key={id} style={{flex:"0 0 auto",minWidth:0,width:"16.666%",display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"8px 2px 10px",border:"none",background:"none",color:tab===id?"#e8621a":"#72727c",cursor:"pointer",fontSize:9,fontFamily:"'Barlow',sans-serif",fontWeight:600,letterSpacing:0.3,textTransform:"uppercase"}} onClick={()=>setTab(id)}>
            {icon}{label}
          </button>
        ))}
      </nav>
...
function CaloriesTab({profile,onSave}){
  ...
  // Adjustable macros — start from calculated values
  const base=useMemo(()=>results?{protein:results.protein,fat:results.fat,carbs:results.carbs}:null,[results]);
  const [macros,setMacros]=useState(null);
  useEffect(()=>{if(base)setMacros(base);},[base?.protein,base?.fat,base?.carbs]);
  ...
  const adjustMacro=(macro,delta)=>{
    setMacros(prev=>{
      const p={...prev};
      const calsPerG={protein:4,fat:9,carbs:4};
      const deltaG=Math.round(delta/calsPerG[macro]);
      const newVal=Math.max(0,p[macro]+deltaG);
      const actualDelta=newVal-p[macro];
      const actualCalDelta=actualDelta*calsPerG[macro];
      p[macro]=newVal;
      if(macro==="protein"){
        // Split cal difference equally between carbs and fats
        const halfCals=actualCalDelta/2;
        p.carbs=Math.max(0,p.carbs-Math.round(halfCals/4));
        p.fat=Math.max(0,p.fat-Math.round(halfCals/9));
      } else if(macro==="fat"){
        // Adjust carbs only
        p.carbs=Math.max(0,p.carbs-Math.round(actualCalDelta/4));
      } else if(macro==="carbs"){
        // Adjust fat only
        p.fat=Math.max(0,p.fat-Math.round(actualCalDelta/9));
      }
      return p;
    });
  };

  const resetMacros=()=>setMacros(base);
  ...
  return(
    <div style={{padding:"14px 14px 32px"}}>
      ...
      <div style={{fontSize:11,color:"#606068",marginBottom:12}}>Slide to adjust. Protein stays fixed when changing carbs or fats.</div>
      <div style={{display:"flex",flexDirection:"column",gap:14,marginBottom:20}}>
        {[["PROTEIN",m.protein,4,"#f472b6","protein",300],["CARBS",m.carbs,4,"#34d399","carbs",500],["FAT",m.fat,9,"#fb923c","fat",180]].map(([n,v,kcalPg,c,key,maxG])=>(
          <div key={n} style={{background:"#26262c",border:`1px solid ${c}22`,borderRadius:10,padding:"12px 14px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
              <div style={{fontSize:11,fontWeight:700,letterSpacing:1,color:c,textTransform:"uppercase"}}>{n}</div>
              <div style={{textAlign:"right"}}>
                <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:24,color:c}}>{v}</span>
                <span style={{fontSize:11,color:"#606068"}}> g · {v*kcalPg} kcal</span>
              </div>
            </div>
            <input type="range" min={0} max={maxG} step={1} value={v}
              onChange={e=>{
                const newVal=parseInt(e.target.value);
                const delta=(newVal-v)*kcalPg;
                setMacros(prev=>{
                  const p={...prev};
                  p[key]=newVal;
                  if(key==="protein"){
                    const halfCals=delta/2;
                    p.carbs=Math.max(0,p.carbs-Math.round(halfCals/4));
                    p.fat=Math.max(0,p.fat-Math.round(halfCals/9));
                  } else if(key==="fat"){
                    p.carbs=Math.max(0,p.carbs-Math.round(delta/4));
                  } else if(key==="carbs"){
                    p.fat=Math.max(0,p.fat-Math.round(delta/9));
                  }
                  return p;
                });
              }}
              style={{width:"100%",accentColor:c,cursor:"pointer",height:20}}
            />
            <div style={{display:"flex",justifyContent:"space-between",marginTop:2}}>
              <span style={{fontSize:9,color:"#505058"}}>0g</span>
              <span style={{fontSize:9,color:"#505058"}}>{maxG}g</span>
            </div>
          </div>
        ))}
      </div>
...
function WeightTab({data,onSave}){
  ...
  const weeklyAvgs=useMemo(()=>{
    ...
    return Object.entries(weeks).sort(([a],[b])=>a.localeCompare(b)).map(([wk,ws])=>({week:wk,avg:Math.round((ws.reduce((s,w)=>s+w,0)/ws.length)*100)/100,entries:ws.length}));
  },[sortedAsc]);
  ...
  return(
    <div style={{padding:"14px 14px 32px"}}>
      <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:22,color:"#f0ece6",letterSpacing:0.5,marginBottom:16}}>WEIGHT LOG</div>
        ...
        <>
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            <div style={statBox("#e8621a")}>
              <div style={{fontSize:10,color:"#c07040",fontWeight:700,letterSpacing:1,marginBottom:4}}>CURRENT</div>
              <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:28,color:"#e8621a",lineHeight:1}}>{currentWeight}<span style={{fontSize:13,fontWeight:500}}>kg</span></div>
              <div style={{fontSize:10,color:"#606068",marginTop:2}}>{fmtDate(latest.date)}</div>
            </div>
            {totalChange!==null&&<div style={statBox(totalChange<0?"#34d399":totalChange>0?"#f87171":"#60a5fa")}>
              ...
              <div style={{fontSize:10,color:"#606068",marginTop:2}}>from {startWeight}kg start</div>
            </div>}
          </div>
          ...
          <div style={{...card,padding:14,marginBottom:14}}>
            ...
            {data?.targetWeight&&toTarget!==null&&(
              <div style={{marginTop:10,padding:"10px 12px",background:Math.abs(toTarget)<1?"#2a1408":"#323238",borderRadius:7,border:"1px solid",borderColor:Math.abs(toTarget)<1?"#5a2c0a":"#44444c",display:"flex",alignItems:"center",gap:8}}>
                {Ic.target}
                <div>
                  <span style={{fontWeight:600,color:toTarget<=0?"#34d399":"#f0ece6"}}>{data.targetWeight}kg target</span>
                  <span style={{color:"#606068",fontSize:13}}> — {toTarget<=0?<span style={{color:"#34d399"}}>Goal reached! 🎉</span>:<span>{Math.abs(toTarget)}kg to go</span>}</span>
                </div>
              </div>
            )}
          </div>
          ...
          {weeklyAvgs.length>1&&(
            <div style={{...card,padding:"14px 8px 14px 4px",marginBottom:14}}>
              <div style={{padding:"0 10px 10px",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:13,color:"#72727c",letterSpacing:1}}>WEEKLY AVERAGE TREND</div>
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={chartData} margin={{top:4,right:16,left:0,bottom:4}}>
                  ...
                  <Tooltip contentStyle={{background:"#323238",border:"1px solid #2a2a32",borderRadius:8,color:"#f0ece6",fontSize:12}} formatter={(v)=>[`${v}kg`,"Avg"]} labelFormatter={l=>`Week: ${l}`}/>
                  <Line type="monotone" dataKey="avg" stroke="#e8621a" strokeWidth={2.5} dot={{fill:"#e8621a",strokeWidth:0,r:3}} activeDot={{r:5,fill:"#e8621a"}}/>
                </LineChart>
              </ResponsiveContainer>
                ...
                {[...weeklyAvgs].reverse().map((w,i)=>{
                  ...
                  return(
                    <div key={w.week} style={{display:"flex",alignItems:"center",padding:"10px 14px",borderTop:i===0?"none":"1px solid #141418"}}>
                      ...
                      <div style={{textAlign:"right"}}>
                        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:20,color:"#e8621a"}}>{w.avg}<span style={{fontSize:11,fontWeight:400}}>kg</span></div>
                        {diff!==null&&<div style={{fontSize:11,color:diff<0?"#34d399":diff>0?"#f87171":"#606068"}}>{diff>0?"+":""}{diff}kg</div>}
                      </div>
                    </div>
                  );
                })}
            ...
            {displayed.map((e,i)=>{
              ...
              return(
                <div key={e.date} style={{display:"flex",alignItems:"center",padding:"9px 14px",borderTop:i===0?"none":"1px solid #141418"}}>
                  ...
                  <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:20,color:"#f0ece6",marginRight:12}}>{e.weight}<span style={{fontSize:11,fontWeight:400}}>kg</span></div>
      ...
      {entries.length===0&&(
        <div style={{textAlign:"center",padding:"40px 0",color:"#505058"}}>
          ...
          <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:18,fontWeight:700,color:"#44444c",marginBottom:6}}>No weigh-ins yet</div>
    ...
    {plans.map(p=>(<div key={p.id} style={{...card,border:p.id===activePlanId?"1px solid #5a8a00":"1px solid #1e1e24"}}>
      <div style={{padding:"12px 14px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <div style={{display:"flex",gap:6}}>
          ...
          <button style={{...btn("danger"),padding:"7px 10px"}} onClick={()=>{if(window.confirm(`Delete "${p.name||"this plan"}"? This cannot be undone.`))onSavePlans(plans.filter(x=>x.id!==p.id));}}>{Ic.trash}</button>
        </div>
      </div>
      <div style={{padding:"0 14px 12px",display:"flex",flexWrap:"wrap",gap:5}}>
