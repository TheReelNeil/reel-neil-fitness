...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const measure=()=>{
      // iOS standalone PWAs inconsistently report window/viewport height — sometimes
      // excluding the status-bar area (793 on an 852pt screen), sometimes not.
      // Add the top inset back, but CAP at the physical screen height so it can
      // never overshoot (which cut the bottom nav off when iOS told the truth).
      const standalone=window.navigator.standalone===true||window.matchMedia?.("(display-mode: standalone)")?.matches;
      const sat=parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--sat"))||0;
      const reported=window.visualViewport?.height??window.innerHeight;
      let h=Math.round(reported);
      if(standalone){
        const scrH=Math.max(window.screen?.height||0,window.screen?.width||0);
        h=Math.round(Math.min(reported+sat,scrH||reported+sat));
      }
